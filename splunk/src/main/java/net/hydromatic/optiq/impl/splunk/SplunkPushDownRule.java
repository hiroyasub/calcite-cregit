...
condition|)
block|{
comment|// can't handle - exit and stop optimizer from calling
comment|// any SplunkUdxRel related optimizations
...
literal|null
return|;
comment|// must be of form field=value
block|}
name|int
name|fieldIndex
init|=
...
literal|false
assert|;
comment|/*         RelNode rel =             new JavaRules.EnumerableTableAccessRel(                 udxRel.getCluster(),                 udxRel.getTable(),                 udxRel.getRowType(),                 udxRel.getServerMofId());          rel = RelOptUtil.createCastRel(rel, udxRel.getRowType(), true);          rel = addProjectionRule(bottomProj, rel);          if (filter != null) {             rel =                 new FilterRel(filter.getCluster(), rel, filter.getCondition());         }          rel = addProjectionRule(topProj, rel);          call.transformTo(rel);         */
block|}
specifier|public
specifier|static
name|String
