...
name|results
decl_stmt|;
comment|// only used for ANTI join: if right input is over, all remaining elements from left are results
specifier|private
name|boolean
name|remainingLeft
decl_stmt|;
...
return|;
block|}
comment|/**      * Only for joinType ANTI: if right input is over, all remaining elements from left are results.      * @param currentLeft current element from left iterator.      */
specifier|private
name|void
name|computeLeftRemainder
parameter_list|(
name|TSource
name|currentLeft
parameter_list|)
block|{
comment|// before reaching this point, we have always performed a leftEnumerator moveNext,
comment|// so we must save the leftEnumerator current item as a result, otherwise it would be lost
name|results
operator|=
name|Linq4j
operator|.
...
condition|)
block|{
comment|// left (and all other items with the same key) are results for anti join
if|if
condition|(
operator|!
name|advanceLeft
