...
return|return
operator|new
name|SortedAggregateEnumerator
argument_list|(
name|enumerable
argument_list|,
name|keySelector
...
condition|)
block|{
comment|// input has been exhausted.
name|isLastMoveNextFalse
operator|=
literal|true
expr_stmt|;
...
condition|)
block|{
comment|// TODO: the implementation assumes accumulatorAdder always produces non-nullable values
comment|// Should a separate boolean field be used to track initialization?
name|curAccumulator
operator|=
name|accumulatorInitializer
operator|.
...
expr_stmt|;
block|}
comment|// reset result because now it can move to next aggregated result.
name|curResult
operator|=
literal|null
expr_stmt|;
...
condition|)
block|{
comment|// current key is different from previous key, get accumulated results and re-create
comment|// accumulator for current key.
name|curResult
operator|=
name|resultSelector
operator|.
...
condition|)
block|{
comment|// current key is the last key.
name|curResult
operator|=
name|resultSelector
operator|.
...
argument_list|)
expr_stmt|;
comment|// no need to keep accumulator for the last key.
name|curAccumulator
operator|=
literal|null
expr_stmt|;
...
begin_comment
comment|/**    * Implementation of nested loop join that builds the complete result as a list    * and then returns it. This is an easy-to-implement solution, but hogs memory.    */
end_comment
...
begin_comment
comment|/**    * Implementation of nested loop join that, unlike {@link #nestedLoopJoinAsList}, does not    * require to build the complete result as a list before returning it. Instead, it iterates    * through the outer enumerable and inner enumerable and returns the results step by step.    * It does not support RIGHT / FULL join.    */
end_comment
...
literal|false
decl_stmt|;
comment|// whether the outerValue has matched an innerValue
specifier|private
annotation|@
name|Nullable
name|TSource
...
name|SEMI
case|:
comment|// return result, and try next outer row
name|state
operator|=
literal|0
expr_stmt|;
...
block|{
comment|// innerEnumerator is over
name|state
operator|=
literal|0
expr_stmt|;
...
condition|)
block|{
comment|// No match detected: outerValue is a result for LEFT / ANTI join
return|return
literal|true
return|;
block|}
...
begin_comment
comment|/**    * Joins two inputs that are sorted on the key.    * Inputs must sorted in ascending order, nulls last.    * @deprecated Use {@link #mergeJoin(Enumerable, Enumerable, Function1, Function1, Function2, JoinType, Comparator)}    */
end_comment
...
begin_comment
comment|/**    * Returns if certain join type is supported by Enumerable Merge Join implementation.    *<p>NOTE: This method is subject to change or be removed without notice.    */
end_comment
...
name|UnsupportedOperationException
argument_list|(
literal|"MergeJoin unsupported for join type "
operator|+
name|joinType
argument_list|)
throw|;
...
begin_comment
comment|/**    * A sort implementation optimized for a sort with a fetch size (LIMIT).    * @param offset how many rows are skipped from the sorted output.    *               Must be greater than or equal to 0.    * @param fetch how many rows are retrieved. Must be greater than or equal to 0.    */
end_comment
...
parameter_list|)
block|{
comment|// As discussed in CALCITE-3920 and CALCITE-4157, this method avoids to sort the complete input,
comment|// if only the first N rows are actually needed. A TreeMap implementation has been chosen,
comment|// so that it behaves similar to the orderBy method without fetch/offset.
comment|// The TreeMap has a better performance if there are few distinct sort keys.
return|return
operator|new
name|AbstractEnumerable
argument_list|<
...
condition|)
block|{
comment|// the current row will never appear in the output, so just skip it
annotation|@
name|KeyFor
argument_list|(
literal|"map"
...
continue|continue;
block|}
comment|// remove last entry from tree map, so that we keep at most 'needed' rows
annotation|@
name|SuppressWarnings
argument_list|(
literal|"argument.type.incompatible"
...
expr_stmt|;
block|}
comment|// add the current element to the map
name|map
operator|.
name|compute
argument_list|(
...
block|}
block|}
comment|// skip the first 'offset' rows by deleting them from the map
if|if
condition|(
name|offset
operator|>
...
condition|)
block|{
comment|// search the key up to (but excluding) which we have to remove entries from the map
name|int
name|skipped
init|=
literal|0
...
condition|)
block|{
comment|// we might need to remove entries from the list
name|List
argument_list|<
name|TSource
argument_list|>
...
condition|)
block|{
comment|// the offset is bigger than the number of rows in the map
return|return
name|Linq4j
operator|.
name|emptyEnumerator
...
begin_comment
comment|/** Enumerator that casts each value.    *    *<p>If the source type {@code F} is not nullable, the target element type    * {@code T} is not nullable. In other words, an enumerable over elements that    * are not null will yield another enumerable over elements that are not null.    *    * @param<F> source element type    * @param<T> element type    */
end_comment
...
name|innerKeySelector
decl_stmt|;
comment|// extra predicate in case of non equi-join, in case of equi-join it will be null
specifier|private
specifier|final
annotation|@
name|Nullable
...
literal|null
decl_stmt|;
comment|// used for LEFT/ANTI join: if right input is over, all remaining elements from left are results
specifier|private
name|boolean
name|remainingLeft
decl_stmt|;
...
return|;
block|}
comment|/** Returns whether the left enumerator was successfully advanced to the next      * element, and it does not have a null key (except for LEFT join, that needs to process      * all elements from left. */
specifier|private
name|boolean
name|leftMoveNext
parameter_list|()
...
condition|)
block|{
name|startLeftOrAntiJoin
argument_list|()
expr_stmt|;
block|}
else|else
block|{
comment|// joinType INNER or SEMI
if|if
condition|(
operator|!
name|leftMoveNext
...
condition|)
block|{
comment|// all remaining items in left are results for anti join
name|remainingLeft
operator|=
literal|true
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// iterate until finding matching keys (or ANTI join results)
for|for
control|(
init|;
condition|;
...
condition|)
block|{
comment|// all remaining items in left are results for left/anti join
name|remainingLeft
operator|=
literal|true
expr_stmt|;
...
condition|)
block|{
comment|// left (and all other items with the same key) are results for left/anti join
if|if
condition|(
operator|!
name|advanceLeft
...
operator|=
operator|new
name|CartesianProductJoinEnumerator
argument_list|<>
argument_list|(
name|resultSelector
argument_list|,
...
continue|continue;
block|}
comment|// SEMI join must not have duplicates, in that case take just one element from rights
name|results
operator|=
name|joinType
operator|==
...
block|{
comment|// we must verify the non equi-join predicate, use nested loop join for that
name|results
operator|=
name|nestedLoopJoin
argument_list|(
...
block|{
comment|// mergeJoin assumes inputs sorted in ascending order with nulls last,
comment|// if we reach a null key, we are done (except LEFT join, that needs to process LHS fully)
break|break;
block|}
name|int
name|c
...
return|;
block|}
comment|/**      * Clears {@code right} list, adds {@code right} into it, and advance right enumerator,      * adding all items with the same key to {@code right} list too, until right enumerator      * is over or a different key is found.      * @return {@code true} if there are still elements to be processed on the right enumerator,      * {@code false} otherwise (right enumerator is over or null key is found).      */
specifier|private
name|boolean
name|advanceRight
parameter_list|(
...
literal|true
return|;
comment|// no need to check duplicates
block|}
...
begin_comment
comment|/**    * Merge Union Enumerable.    * Performs a union (or union all) of all its inputs (which must be already sorted),    * respecting the order.    *    * @param sources input enumerables (must be already sorted)    * @param sortKeySelector sort key selector    * @param sortComparator sort comparator to decide the next item    * @param all whether duplicates will be considered or not    * @param equalityComparer {@link EqualityComparer} to control duplicates,    *                         only used if {@code all} is {@code false}    * @param<TSource> record type    * @param<TKey> sort key    */
end_comment
