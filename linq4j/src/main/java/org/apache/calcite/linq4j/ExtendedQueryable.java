...
begin_comment
comment|/**  * Extension methods in Queryable.  *  * @param<TSource> Element type  */
end_comment
...
parameter_list|)
function_decl|;
comment|/**    * Groups the elements of a sequence according to a    * specified key selector function and creates a result value from    * each group and its key.    *    *<p>NOTE: Renamed from {@code groupBy} to distinguish from    * {@link #groupBy(org.apache.calcite.linq4j.tree.FunctionExpression, org.apache.calcite.linq4j.tree.FunctionExpression)},    * which has the same erasure.</p>    */
parameter_list|<
name|TKey
parameter_list|,
name|TResult
...
parameter_list|)
function_decl|;
comment|/**    * Projects each element of a sequence into a new    * form by incorporating the element's index.    *    *<p>NOTE: Renamed from {@code select} because had same erasure as    * {@link #select(org.apache.calcite.linq4j.tree.FunctionExpression)}.</p>    */
parameter_list|<
name|TResult
parameter_list|>
name|Queryable
...
parameter_list|)
function_decl|;
comment|/**    * Projects each element of a sequence to an    * {@code Enumerable<TSource>} and combines the resulting sequences into one    * sequence. The index of each source element is used in the    * projected form of that element.    *    *<p>NOTE: Renamed from {@code selectMany} because had same erasure as    * {@link #selectMany(org.apache.calcite.linq4j.tree.FunctionExpression)}</p>    */
parameter_list|<
name|TResult
parameter_list|>
name|Queryable
...
parameter_list|)
function_decl|;
comment|/**    * Projects each element of a sequence to an    * {@code Enumerable<TSource>} and invokes a result selector function on each    * element therein. The resulting values from each intermediate    * sequence are combined into a single, one-dimensional sequence    * and returned.    *    *<p>NOTE: Renamed from {@code selectMany} because had same erasure as    * {@link #selectMany(org.apache.calcite.linq4j.tree.FunctionExpression, org.apache.calcite.linq4j.tree.FunctionExpression)}    */
parameter_list|<
name|TCollection
parameter_list|,
name|TResult
...
begin_comment
comment|// End ExtendedQueryable.java
end_comment
