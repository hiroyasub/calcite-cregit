...
begin_comment
comment|/**  * CalcRelSplitter operates on a {@link CalcRel} with multiple {@link RexCall}  * sub-expressions that cannot all be implemented by a single concrete {@link  * RelNode}.  *  *<p>For example, the Java and Fennel calculator do not implement an identical  * set of operators. The CalcRel can be used to split a single CalcRel with  * mixed Java- and Fennel-only operators into a tree of CalcRel object that can  * each be individually implemented by either Java or Fennel.and splits it into  * several CalcRel instances.  *  *<p>Currently the splitter is only capable of handling two "rel types". That  * is, it can deal with Java vs. Fennel CalcRels, but not Java vs. Fennel vs.  * some other type of CalcRel.  *  *<p>See {@link org.eigenbase.rel.rules.WindowedAggSplitterRule} for an example  * of how this class is used.  */
end_comment
...
index|]
decl_stmt|;
comment|// The reltype of a level is given by
...
condition|)
block|{
comment|// This is a new level. We can use any reltype we like.
for|for
control|(
name|int
name|relTypeOrdinal
...
condition|)
block|{
comment|// Success. We have found a reltype where we can
...
condition|)
block|{
comment|// Cannot implement for any reltype.
throw|throw
name|Util
operator|.
name|newInternal
...
condition|)
block|{
comment|// Cannot implement this expression in this reltype;
