...
begin_comment
comment|/**  * A relational expression implements the interface<code>ConverterRel</code> to  * indicate that it converts a physical attribute, or {@link  * org.eigenbase.relopt.RelTrait trait}, of a relational expression from one  * value to another.  *  *<p>A typical example of a trait is {@link CallingConvention calling  * convention}, and {@link net.sf.farrago.query.FennelToIteratorConverter} is an  * example of a relational expression which converts that trait.</p>  *  *<p>Sometimes this conversion is expensive; for example, to convert a  * non-distinct to a distinct object stream, we have to clone every object in  * the input.</p>  *  *<p>A converter does not change the logical expression being evaluated; after  * conversion, the number of rows and the values of those rows will still be the  * same. By declaring itself to be a converter, a relational expression is  * telling the planner about this equivalence, and the planner groups  * expressions which are logically equivalent but have different physical traits  * into groups called<code>RelSet</code>s.  *  *<p>In principle one could devise converters which change multiple traits  * simultaneously (say change the sort-order and the physical location of a  * relational expression). In which case, the method {@link #getInputTraits()}  * would return a {@link org.eigenbase.relopt.RelTraitSet}. But for simplicity,  * this class only allows one trait to be converted at a time; all other traits  * are assumed to be preserved.</p>  *  * @author jhyde  * @version $Id$  * @since Dec 12, 2007  */
end_comment
