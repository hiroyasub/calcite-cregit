...
parameter_list|)
block|{
comment|// The reference file for class "com.foo.Bar" is "com/foo/Bar.ref.xml"
name|String
name|rest
init|=
name|clazz
...
condition|)
block|{
comment|// set in base repos; don't override
block|}
else|else
block|{
name|set
...
literal|"' overrides a "
operator|+
literal|"testcase in the base repository, but does "
operator|+
...
comment|// looks like a testcase name, e.g. "testFoo".
specifier|final
...
name|RuntimeException
argument_list|(
literal|"no testcase on current callstack"
argument_list|)
throw|;
block|}
else|else
...
expr_stmt|;
block|}
comment|//            writeNode(((Document) node).getDocumentElement(),
comment|// out);
break|break;
case|case
name|Node
