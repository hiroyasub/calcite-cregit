...
comment|/**      * Creates a converter.      *      * @param validator Validator      * @param schema Schema      * @param env Environment      * @param connection Connection      * @param rexBuilder Rex builder      *      * @pre connection != null      */
specifier|public
name|SqlToRelConverter
parameter_list|(
name|SqlValidator
...
literal|0
decl_stmt|;
comment|// number of non-dup exprs seen so far
for|for
control|(
name|int
name|i
...
block|{
comment|// TODO jvs 1-May-2006: Generalize
comment|// RexUtil.andRexNodeList and use that.
name|result
operator|=
name|rexBuilder
operator|.
...
parameter_list|)
block|{
comment|// DESC keyword, e.g. 'select a, b from t order by a desc'.
if|if
condition|(
name|orderItem
operator|instanceof
...
expr_stmt|;
block|}
comment|// Walk the expresion list and get default values for any columns
...
comment|/**      * A<code>SchemaCatalogReader</code> looks up catalog information from a      * {@link org.eigenbase.relopt.RelOptSchema schema object}.      */
specifier|public
specifier|static
class|class
name|SchemaCatalogReader
...
literal|null
argument_list|,
literal|"aggregate fun must occur in aggregation mode"
argument_list|)
expr_stmt|;
return|return
name|agg
...
name|relOffsetList
decl_stmt|;
comment|/**          * Creates a LookupContext with a single input relational expression.          *          * @param rel Relational expression          * @param systemFieldCount Number of system fields          */
name|LookupContext
parameter_list|(
name|RelNode
name|rel
...
argument_list|)
decl_stmt|;
comment|// The fennel support for windowed Agg MIN/MAX only
comment|// supports BIGINT and DOUBLE numeric types. If the
comment|// expression result is numeric but not correct then pre
comment|// and post CAST the data.  Example with INTEGER
comment|// CAST(MIN(CAST(exp to BIGINT)) to INTEGER)
name|SqlFunction
name|histogramOp
init|=
name|isUnboundedPreceding
...
argument_list|)
decl_stmt|;
comment|// If a window contains only the current row, treat it as physical.
comment|// (It could be logical too, but physical is simpler to implement.)
name|boolean
name|physical
init|=
name|window
