...
return|;
block|}
comment|/**      * If<code>selectItem</code> is "*" or "TABLE.*", expands it and returns      * true; otherwise writes the unexpanded item.      *      * @param selectItem Select-list item      * @param select Containing select clause      * @param selectItems List that expanded items are written to      * @param aliases List of aliases      * @param types List of data types in alias order      * @param includeSystemVars If true include system vars in lists      *      * @return Whether the node was expanded      */
specifier|private
name|boolean
name|expandSelectItem
parameter_list|(
...
parameter_list|)
block|{
comment|// For large lists, it's more efficient to build a set to do a quick
comment|// check for duplicates before we do an O(n^2) search.
if|if
condition|(
operator|(
name|list
...
name|i
return|;
comment|// ordinal of the later item
block|}
block|}
block|}
return|return
