...
name|exprs
decl_stmt|;
comment|// REVIEW: angel 2006-08-27 Commented out the following section
comment|// piece of code, not sure what its purpose is, but it was causing
comment|// errors
comment|/*         if (type instanceof IntervalSqlType) {             //if (op instanceof SqlDatetimeSubtractionOperator) {             //    op = SqlStdOperatorTable.minusOperator;             //}             int count = 0;             for (int i = 0; i< exprs.length; i++) {                 if ((exprs[i] instanceof RexLiteral)&& (exprs[i].getType() instanceof IntervalSqlType)&& (exprs[i].getType().getIntervalQualifier() != null)) {                     exprs[i] = null;                     continue;                 }                 count++;             }             fixExprs = new RexNode[count];             for (int i = 0; i< exprs.length; i++) {                 if (exprs[i] == null) {                     continue;                 }                 fixExprs[i] = exprs[i];             }         }  */
return|return
operator|new
name|RexCall
argument_list|(
