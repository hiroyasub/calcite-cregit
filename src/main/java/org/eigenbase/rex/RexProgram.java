...
assert|;
block|}
comment|/**      * Creates a program from lists of expressions.      *      * @param inputRowType Input row type      * @param exprList List of common expressions      * @param projectRefList List of projection expressions      * @param condition Condition expression. If null, calculator does not      * filter rows      * @param outputRowType Description of the row produced by the program      *      * @pre !containCommonExprs(exprList)      * @pre !containForwardRefs(exprList)      * @pre !containNonTrivialAggs(exprList)      */
specifier|public
name|RexProgram
parameter_list|(
name|RelDataType
...
comment|// and there is no further reduction into smaller common subexpressions
comment|// possible"?
comment|/**      * Returns the common sub-expressions of this program.      *      *<p>The list is never null but may be empty; each the expression in the      * list is not null; and no further reduction into smaller common      * subexpressions is possible.      *      * @post return != null      * @post !containCommonExprs(exprs)      */
specifier|public
name|List
argument_list|<
name|RexNode
...
return|;
block|}
comment|/**      * Helper method for 'explain' functionality. Creates a list of all      * expressions (common, project, and condition)      *      * @deprecated Not used      */
specifier|public
name|RexNode
index|[]
name|flatten
...
return|;
block|}
comment|/**      * Writes an explanation of the expressions in this program to a plan      * writer.      *      * @param rel Relational expression which owns this program      * @param pw Plan writer      */
specifier|public
name|void
name|explainCalc
parameter_list|(
...
expr_stmt|;
block|}
comment|// Relational expressions which contain a program should report their
comment|// children in a different way than getChildExps().
assert|assert
name|rel
operator|.
name|getChildExps
...
expr_stmt|;
block|}
comment|/**      * Collects the expressions in this program into a list of terms and values.      *      * @param prefix Prefix for term names, usually the empty string, but useful      * if a relational expression contains more than one program      * @param termList Output list of terms      * @param valueList Output list of expressions      */
specifier|public
name|void
name|collectExplainTerms
parameter_list|(
...
block|}
block|}
comment|/**      * Returns the number of expressions at the front of an array which are      * simply projections of the same field.      */
specifier|private
specifier|static
name|int
name|countTrivial
...
return|;
block|}
comment|/**      * Gets reference counts for each expression in the program, where the      * references are detected from later expressions in the same program, as      * well as the project list and condition. Expressions with references      * counts greater than 1 are true common subexpressions.      *      * @return array of reference counts; the ith element in the returned array      * is the number of references to getExprList()[i]      */
specifier|public
name|int
index|[]
