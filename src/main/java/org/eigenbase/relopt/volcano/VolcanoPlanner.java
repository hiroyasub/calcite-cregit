...
argument_list|)
decl_stmt|;
comment|// Fire this rule early so we don't waste time fiddling with
comment|// alternate join implementations.  The MDR queries are tuned
comment|// to want this rule's output.
name|preProcessMdrPhaseRules
operator|.
name|add
argument_list|(
literal|"MedMdrJoinRule"
argument_list|)
expr_stmt|;
name|preProcessMdrPhaseRules
operator|.
...
argument_list|)
decl_stmt|;
comment|// Cleanup pass to remove trivial projects and merge calcs.
comment|// Helps the planner concentrate on these easy tasks.
name|cleanupPhaseRules
operator|.
name|add
argument_list|(
literal|"RemoveTrivialProjectRule"
argument_list|)
expr_stmt|;
name|cleanupPhaseRules
operator|.
...
return|;
block|}
comment|// REVIEW: SWZ: 3/1/2005: No one calls this.  Remove?
comment|// todo: pre-compute
specifier|public
name|RelOptRuleOperand
index|[]
name|getConversionOperands
parameter_list|(
...
return|;
block|}
comment|// REVIEW: SWZ: 3/5/2005: This is probably redundant.  The call
comment|// to changeTraitsUsingConverters should create a string of
comment|// AbstractConverter if none of the conversions is currently possible.
name|RelNode
name|converter
init|=
name|rel
...
expr_stmt|;
block|}
comment|// REVIEW: SWZ: 3/5/2005: Why is (was) this only done for abstract
comment|// converters?  Seems to me, the caller has to register the
comment|// conversion in the end anyway.
return|return
name|register
argument_list|(
name|converter
...
expr_stmt|;
block|}
comment|/**      * Finds RelSubsets in the plan that contain only rels of {@link      * CallingConvention#NONE} and boosts their importance by 25%.      */
specifier|private
name|void
name|injectImportanceBoost
parameter_list|()
...
argument_list|()
operator|+
literal|"] but is not registered as such"
argument_list|)
throw|;
block|}
block|}
...
argument_list|)
expr_stmt|;
comment|// Remove backlinks from children.
for|for
control|(
name|RelNode
name|input
...
name|existed
operator|:
literal|"rel was not known to its subset"
assert|;
name|existed
operator|=
name|subset
