...
argument_list|)
decl_stmt|;
comment|// Build the input program.
specifier|final
name|RexProgram
name|inProgram
init|=
...
argument_list|)
decl_stmt|;
comment|// Make sure the orderkey is among the input expressions from the child.
comment|// Merged program creates intermediate rowtype below. OutputProgram is
comment|// then built based on the intermediate row which also holds winAgg
comment|// results. orderKey is resolved before intermediate rowtype is
comment|// constructed and "orderKey ordinal" may point to wrong offset within
comment|// intermediate row. dtbug #2209.
for|for
control|(
name|RexNode
name|expr
...
condition|)
block|{
comment|// do not merge inCalc and winAggRel.
return|return;
block|}
block|}
block|}
block|}
comment|// The purpose of the input program is to provide the expressions
comment|// needed by all of the aggregate functions. Its outputs are (a) all
comment|// of the input fields, followed by (b) the input expressions for the
...
assert|;
block|}
comment|// Put all these programs together in the final relational expression.
name|WindowRel
operator|.
name|Window
index|[]
...
argument_list|)
decl_stmt|;
comment|// default case; normal java to show what it means
...
argument_list|()
decl_stmt|;
comment|// copy over other traits from the child
for|for
control|(
name|i
operator|=
...
block|}
block|}
comment|// convert to the traits of the calling rel to which we are equivalent
comment|// and thus must have the same traits
name|RelNode
name|mergedFennelCalcRel
init|=
name|fennelCalcRel
decl_stmt|;
if|if
...
block|}
block|}
comment|/**    * Relational expression which computes windowed aggregates.    *    *<p>A window rel can handle several window aggregate functions, over several    * partitions, with pre- and post-expressions, and an optional post-filter.    * Each of the partitions is defined by a partition key (zero or more columns)    * and a range (logical or physical). The partitions expect the data to be    * sorted correctly on input to the relational expression.    *    *<p>Rules:    *    *<ul>    *<li>{@link net.hydromatic.optiq.rules.java.JavaRules.EnumerableWindowRule} creates this from a {@link org.eigenbase.rel.CalcRel}</li>    *<li>{@link org.eigenbase.rel.rules.WindowedAggSplitterRule} decomposes a {@link org.eigenbase.rel.CalcRel} which    * contains windowed aggregates into a {@link net.hydromatic.optiq.rules.java.JavaRules.EnumerableWindowRel} and zero or more    * {@link org.eigenbase.rel.CalcRel}s which do not contain windowed aggregates</li>    *</ul>    *</p>    */
specifier|public
specifier|static
class|class
name|EnumerableWindowRel
...
comment|// components (windows, partitions, and aggregate functions). There is
comment|// no I/O cost.
comment|//
