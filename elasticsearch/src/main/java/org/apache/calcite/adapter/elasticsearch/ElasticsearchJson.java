...
begin_comment
comment|/**  * Internal objects (and deserializers) used to parse elastic search results  * (which are in JSON format).  *  *<p>Since we're using basic row-level rest client http response has to be  * processed manually using JSON (jackson) library.  */
end_comment
...
block|{
comment|/**    * Used as special aggregation key for missing values (documents which are missing a field).    * Buckets with that value are then converted to {@code null}s in flat tabular format.    * @see<a href="https://www.elastic.co/guide/en/elasticsearch/reference/current/search-aggregations-metrics-sum-aggregation.html">Missing Value</a>    */
specifier|static
specifier|final
name|JsonNode
name|MISSING_VALUE
...
condition|)
block|{
comment|// publish one value of the row
name|RowKey
name|key
init|=
operator|new
...
argument_list|(
literal|"'value' field not present in this aggregation"
argument_list|)
throw|;
block|}
return|return
