...
begin_comment
comment|/**  * Utility class to generate elastic search queries. Most query builders have  * been copied from ES distribution. The reason we have separate definition is  * high-level client dependency on core modules (like lucene, netty, XContent etc.) which  * is not compatible between different major versions.  *  *<p>Jackson API is used to generate ES query as JSON document.  */
end_comment
...
block|}
block|}
comment|/**    * Write single simple (scalar) value (string, number, boolean or null) to json output.    *    * @param generator api to generate JSON document    * @param value JSON value to write    * @throws IOException if can't write to output    */
specifier|private
specifier|static
name|void
name|writeScalar
...
condition|)
block|{
comment|// write numbers as string
name|generator
operator|.
name|writeNumber
argument_list|(
