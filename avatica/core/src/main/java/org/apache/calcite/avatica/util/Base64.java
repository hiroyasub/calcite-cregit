...
condition|)
block|{
comment|// If breaking lines and the last byte falls right at
comment|// the line length (76 bytes per line), there will be
comment|// one extra byte, and the array will need to be resized.
comment|// Not too bad of an estimate on array size, I'd say.
name|byte
index|[]
name|finalOut
init|=
...
argument_list|)
expr_stmt|;
comment|//        } catch( java.io.IOException ex ) {
comment|//            assert false : "IOExceptions only come from GZipping, which is turned off: " + ex.getMessage();
comment|//        }
return|return
name|decoded
return|;
...
name|ROOT
argument_list|,
literal|"Bad Base64 input character decimal %d in array position %d"
argument_list|,
operator|(
operator|(
name|int
...
index|]
decl_stmt|;
comment|// Need max() for math on small files (v2.2.1); Need +1 for a few corner cases (v2.3.5)
name|int
name|length
init|=
literal|0
...
comment|/**      * Constructs a {@link Base64.InputStream} in      * either ENCODE or DECODE mode.      *<p>      * Valid options:<pre>      *   ENCODE or DECODE: Encode or Decode as data is read.      *   DO_BREAK_LINES: break lines at 76 characters      *     (only meaningful when encoding)      *</pre>      *<p>      * Example:<code>new Base64.InputStream( in, Base64.DECODE )</code>      *      *      * @param in the<tt>java.io.InputStream</tt> from which to read data.      * @param options Specified options      * @see Base64#ENCODE      * @see Base64#DECODE      * @see Base64#DO_BREAK_LINES      * @since 2.0      */
specifier|public
name|InputStream
parameter_list|(
name|java
