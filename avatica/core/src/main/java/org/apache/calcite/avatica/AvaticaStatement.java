...
name|RuntimeException
argument_list|(
literal|"Failed to successfully execute query after "
operator|+
name|connection
operator|.
name|maxRetriesPerExecute
...
name|RuntimeException
argument_list|(
literal|"Failed to successfully execute batch update after "
operator|+
name|connection
operator|.
name|maxRetriesPerExecute
...
parameter_list|()
block|{
comment|// Invalidate the old statement
name|connection
operator|.
name|statementMap
operator|.
...
argument_list|)
decl_stmt|;
comment|// Cache it in the connection
name|connection
operator|.
name|statementMap
operator|.
...
argument_list|)
expr_stmt|;
comment|// Update the local state and try again
name|this
operator|.
name|handle
operator|=
...
expr_stmt|;
block|}
comment|/**    * Re-initialize the ResultSet on the server with the given state.    * @param state The ResultSet's state.    * @param offset Offset into the desired ResultSet    * @return True if the ResultSet has more results, false if there are no more results.    */
specifier|protected
name|boolean
name|syncResults
parameter_list|(
