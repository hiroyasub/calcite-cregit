...
try|try
block|{
comment|// Get the handle from the statement
name|Meta
operator|.
name|StatementHandle
name|handle
...
return|;
block|}
comment|/** Returns or creates a slot whose state can be changed to cancel a    * statement. Statements will receive the same slot if and only if their id    * is the same. */
specifier|public
name|AtomicBoolean
name|getCancelFlag
parameter_list|(
...
name|upgrade
argument_list|(
literal|"after dropping JDK 1.7, use Map.computeIfAbsent"
argument_list|)
expr_stmt|;
synchronized|synchronized
init|(
