...
argument_list|)
expr_stmt|;
comment|// Add the deprecated 'value' repeated attribute for backwards compat
name|columnBuilder
operator|.
name|addValue
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// Let TypedValue handle the serialization for us.
name|TypedValue
operator|.
name|toProto
argument_list|(
...
parameter_list|)
block|{
comment|// ByteString is a single case where TypedValue is representing the data differently
comment|// (in its "local" form) than Frame does. We need to unwrap the Base64 encoding.
if|if
condition|(
name|Common
operator|.
...
condition|)
block|{
comment|// Protobuf is sending native bytes (not b64) across the wire. B64 bytes is only for
comment|// TypedValue's benefit
return|return
name|protoElement
operator|.
name|getBytesValue
...
return|;
block|}
comment|// Again, let TypedValue deserialize things for us.
return|return
name|TypedValue
operator|.
name|fromProto
