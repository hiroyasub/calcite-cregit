...
return|;
block|}
comment|/** Returns a copy of the current parameter values.    * @return A copied list of the parameter values    */
specifier|protected
name|List
argument_list|<
name|TypedValue
...
parameter_list|()
block|{
comment|// For implementing batch update, we need to make a copy of slots, not just a thin reference
comment|// to it as as list. Otherwise, subsequent setFoo(..) calls will alter the underlying array
comment|// and modify our cached TypedValue list.
name|List
argument_list|<
name|TypedValue
argument_list|>
...
name|SQLException
block|{
comment|// Need to copy the parameterValues into a new list, not wrap the array in a list
comment|// as getParameterValues does.
name|this
operator|.
name|parameterValueBatch
operator|.
...
name|SQLException
block|{
comment|// Overriding the implementation in AvaticaStatement.
try|try
block|{
return|return
name|getConnection
