...
argument_list|)
decl_stmt|;
comment|//    final Integer versionMaintenance = parseIntString(versionArray
comment|//      .get(2).toString());
comment|//    final Integer versionBuild = parseIntString(versionArray
comment|//      .get(3).toString());
for|for
control|(
name|String
name|operation
...
try|try
block|{
comment|// Changed in version 2.6: The db.collection.aggregate() method
comment|// returns a cursor
comment|// and can return result sets of any size.
comment|// See: http://docs.mongodb.org/manual/core/aggregation-pipeline
if|if
condition|(
name|versionMajor
operator|>
...
block|{
comment|// Pre MongoDB version 2.6
...
return|;
block|}
comment|/** Executes an "aggregate" operation for pre-2.6 mongo servers.    *<p>Return document is limited to 4M or 16M in size depending on    * version of mongo<p>Helper method for    * {@link net.hydromatic.optiq.impl.mongodb.MongoTable#aggregate}    *</p>    * @param dbCollection    * @param first the first aggregate action    * @param rest the rest of the aggregate actions    * @return AggregationOutput */
specifier|private
name|AggregationOutput
name|aggregateOldWay
parameter_list|(
