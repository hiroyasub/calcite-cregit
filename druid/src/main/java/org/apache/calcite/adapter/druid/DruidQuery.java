...
argument_list|)
comment|/**      * formatDateString is used to format timestamp values to druid format using      * {@link DruidQuery.Translator#dateFormatter}. This is needed when pushing timestamp      * comparisons to druid using TimeFormatExtractionFunction that returns a string value.      */
name|String
name|translate
parameter_list|(
name|RexNode
...
block|{
comment|// Case when we are passing to druid as an extractionFunction
comment|// Need to format the timestamp String in druid format.
name|TimestampString
name|timestampString
init|=
name|DruidDateTimeUtils
