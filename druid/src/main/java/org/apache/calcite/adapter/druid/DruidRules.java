...
return|;
block|}
comment|/**      * Attempts to optimize any aggregations with filters in the DruidQuery by      * 1. Trying to abstract common filters out into the "filter" field      * 2. Eliminating expressions that are always true or always false when possible      * 3. ANDing aggregate filters together with the outer filter to allow for pruning of data      * Should be called before pushing both the aggregate and project into Druid.      * Assumes that at least one aggregate call has a filter attached to it      * */
specifier|private
name|DruidQuery
name|optimizeFilteredAggregations
parameter_list|(
