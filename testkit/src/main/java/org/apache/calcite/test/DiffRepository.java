...
argument_list|(
literal|"Document Builder configuration failed"
argument_list|,
name|e
argument_list|)
throw|;
...
argument_list|)
expr_stmt|;
comment|// Don't write a log file yet -- as far as we know, it's still identical.
block|}
catch|catch
parameter_list|(
name|IOException
...
argument_list|(
literal|"Actual and reference files differ. "
operator|+
literal|"If you are adding new tests, replace the reference file with the "
operator|+
literal|"current actual file, after checking its content."
operator|+
literal|"\ndiff "
operator|+
name|logFile
...
return|;
block|}
comment|/** Returns the diff repository, checking that it is not null.    *    *<p>If it is null, throws {@link IllegalArgumentException} with a message    * informing people that they need to change their test configuration. */
specifier|public
specifier|static
name|DiffRepository
name|castNonNull
...
operator|+
literal|"DiffRepository, set it in your test's fixture() method"
argument_list|)
throw|;
block|}
...
condition|)
block|{
comment|// Document has not been modified since last write.
return|return;
block|}
try|try
block|{
...
block|}
block|}
comment|// Are new children equal to old?
if|if
condition|(
name|equalList
argument_list|(
...
return|;
block|}
comment|// Replace old children with new children
name|removeAllChildren
argument_list|(
name|element
argument_list|)
...
return|;
block|}
comment|/** Returns whether two lists of nodes are equal. */
specifier|private
specifier|static
name|boolean
name|equalList
