...
return|;
block|}
comment|/**    * Runs an external application process.    *    * @param pb        ProcessBuilder for the application    * @param logger    if not null, command and exit status will be logged here    * @param appInput  if not null, data will be copied to application's stdin    * @param appOutput if not null, data will be captured from application's    *                  stdout and stderr    * @return application process exit value    */
specifier|static
name|int
name|runAppProcess
parameter_list|(
...
return|;
block|}
comment|/**    * Determines whether a block of SQL is a SELECT statement.    */
specifier|private
name|boolean
name|isSelect
parameter_list|(
...
comment|// directory.
comment|//
comment|// WARNING: ProcessBuilder is security-sensitive. Its use is currently
comment|// safe because this code is under "core/test". Developers must not move
comment|// this code into "core/main".
name|ProcessBuilder
name|pb
init|=
operator|new
