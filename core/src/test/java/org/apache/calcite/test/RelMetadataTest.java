...
name|convertSql
argument_list|(
literal|"select 'empid' from emp where 'empid'> 10"
argument_list|,
name|metadataConfig
argument_list|)
decl_stmt|;
...
expr_stmt|;
block|}
comment|/**    * A metadata handler that works to create a cyclic loop.    */
specifier|public
specifier|static
class|class
name|Looping1
...
parameter_list|)
block|{
comment|// buggy since it uses rel, instead of the input for selectivity operation, causing loop.
return|return
name|mq
operator|.
name|getSelectivity
...
parameter_list|()
lambda|->
literal|"we are checking a join against its own collation, "
operator|+
literal|"fetch=null, offset=null => checkInputForCollationAndLimit must be true. join="
operator|+
name|join
argument_list|)
expr_stmt|;
