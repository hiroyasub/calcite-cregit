...
block|{
return|return
literal|"CheckResultCommand [sql: "
operator|+
name|sqlCommand
operator|.
name|sql
...
name|OUTPUTFORMAT
block|}
comment|/** Command that defines the current SQL statement.    *    * @see CheckResultCommand    * @see ExplainCommand    */
class|class
name|SetCommand
extends|extends
name|SimpleCommand
...
condition|)
block|{
comment|// If "skip" is set, stay in current (disabled) mode.
name|newExecute
operator|=
name|oldExecute
expr_stmt|;
...
block|{
comment|// If "enable" is true, stay in the current mode.
name|newExecute
operator|=
name|enable
expr_stmt|;
...
name|Exception
block|{
comment|// We handle all RuntimeExceptions, all Exceptions, and a limited number
comment|// of Errors. If we don't understand an Error (e.g. OutOfMemoryError)
comment|// then we print it out, then abort.
for|for
control|(
name|Command
name|command
...
parameter_list|)
block|{
comment|// We handle a limited number of errors.
name|e
operator|=
name|e0
expr_stmt|;
