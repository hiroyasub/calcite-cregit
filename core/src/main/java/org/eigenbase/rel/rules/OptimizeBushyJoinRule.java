...
begin_comment
comment|/**  * Planner rule that finds an approximately optimal ordering for join operators  * using a heuristic algorithm.  *  *<p>It is triggered by the pattern {@link ProjectRel} ({@link MultiJoinRel}).  *  *<p>It is similar to {@link org.eigenbase.rel.rules.LoptOptimizeJoinRule}.  * {@code LoptOptimizeJoinRule} is only capable of producing left-deep joins;  * this rule is capable of producing bushy joins.  *  *<p>TODO:  *<ol>  *<li>Join conditions that touch 1 factor.  *<li>Join conditions that touch 3 factors.  *<li>More than 1 join conditions that touch the same pair of factors,  *       e.g. {@code t0.c1 = t1.c1 and t1.c2 = t0.c3}  *</ol>  */
end_comment
...
comment|// Therefore, for now, the factors that are merged are exactly the factors
comment|// on this edge.
name|BitSet
name|merged
init|=
name|bestEdge
...
expr_stmt|;
block|}
comment|/** Removes the element of a list at a given ordinal, moving the last element    * into its place. This is an efficient means of removing an element from an    * array list if you do not mind the order of the list changing. */
specifier|private
specifier|static
parameter_list|<
name|E
