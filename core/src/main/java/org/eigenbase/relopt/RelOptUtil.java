...
return|;
block|}
comment|/**    * Creates a plan suitable for use in<code>EXISTS</code> or<code>IN</code>    * statements.    *    * @see org.eigenbase.sql2rel.SqlToRelConverter#convertExists    *    * @param seekRel    A query rel, for example the resulting rel from 'select *    *                   from emp' or 'values (1,2,3)' or '('Foo', 34)'.    * @param subqueryType Sub-query type    * @param logic  Whether to use 2- or 3-valued boolean logic    * @param needsOuterJoin Whether query needs outer join    *    * @return A pair of a relational expression which outer joins a boolean    * condition column, and a numeric offset. The offset is 2 if column 0 is    * the number of rows and column 1 is the number of rows with not-null keys;    * 0 otherwise.    */
specifier|public
specifier|static
name|Pair
argument_list|<
...
return|;
block|}
comment|/**    * Creates a relational expression which projects an array of expressions,    * and optionally optimizes.    *    *<p>The result may not be a {@link org.eigenbase.rel.ProjectRel}. If the    * projection is trivial,<code>child</code> is returned directly; and future    * versions may return other formulations of expressions, such as    * {@link org.eigenbase.rel.CalcRel}.    *    * @param child      input relational expression    * @param exprs      list of expressions for the input columns    * @param fieldNames aliases of the expressions, or null to generate    * @param optimize   Whether to return<code>child</code> unchanged if the    *                   projections are trivial.    */
specifier|public
specifier|static
name|RelNode
name|createProject
...
return|;
block|}
comment|/**    * Creates a relational expression which permutes the output fields of a    * relational expression according to a permutation.    *    *<p>Optimizations:</p>    *    *<ul>    *<li>If the relational expression is a {@link org.eigenbase.rel.CalcRel} or    * {@link org.eigenbase.rel.ProjectRel} that is already acting as a    * permutation, combines the new permutation with the old;</li>    *    *<li>If the permutation is the identity, returns the original relational    * expression.</li>    *</ul>    *    *<p>If a permutation is combined with its inverse, these optimizations    * would combine to remove them both.    *    * @param rel         Relational expression    * @param permutation Permutation to apply to fields    * @param fieldNames  Field names; if null, or if a particular entry is null,    *                    the name of the permuted field is used    * @return relational expression which permutes its input fields    */
specifier|public
specifier|static
name|RelNode
name|permute
