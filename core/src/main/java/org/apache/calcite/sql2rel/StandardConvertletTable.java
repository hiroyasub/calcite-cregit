...
argument_list|)
expr_stmt|;
comment|// DATE(string) is equivalent to CAST(string AS DATE),
comment|// but other DATE variants are treated as regular functions.
name|registerOp
argument_list|(
name|SqlLibraryOperators
operator|.
...
block|}
block|}
comment|/** Converts ALL or SOME operators. */
specifier|private
specifier|static
name|RexNode
name|convertQuantifyOperator
...
argument_list|()
decl_stmt|;
comment|// arg.getType() may be ANY
if|if
condition|(
name|argComponentType
operator|==
...
condition|)
block|{
comment|// skip set semantic table node of table function
name|operandList
operator|=
name|call
operator|.
...
block|}
block|}
comment|/** Convertlet that handles the 3-argument {@code TIMESTAMPADD} function    * and the 2-argument BigQuery-style {@code TIMESTAMP_ADD} function. */
specifier|private
specifier|static
class|class
name|TimestampAddConvertlet
...
comment|// TIMESTAMP_ADD(timestamp, interval)
comment|//  => timestamp + interval
comment|// "timestamp" may be of type TIMESTAMP or TIMESTAMP WITH LOCAL TIME ZONE.
specifier|final
name|RexBuilder
name|rexBuilder
init|=
...
literal|2
case|:
comment|// BigQuery-style 'TIMESTAMP_ADD(timestamp, interval)'
specifier|final
name|SqlBasicCall
name|operandCall
init|=
...
break|break;
default|default:
comment|// JDBC-style 'TIMESTAMPADD(unit, count, timestamp)'
name|qualifier
operator|=
name|call
operator|.
...
condition|)
block|{
comment|// Custom time frames have a different path. They are kept as names,
comment|// and then handled by Java functions such as
comment|// SqlFunctions.customTimestampAdd.
specifier|final
name|RexLiteral
name|timeFrameName
init|=
...
argument_list|)
decl_stmt|;
comment|// If the TIMESTAMPADD call has type TIMESTAMP and op2 has type DATE
comment|// (which can happen for sub-day time frames such as HOUR), cast op2 to
comment|// TIMESTAMP.
specifier|final
name|RexNode
name|op2b
...
block|}
block|}
comment|/** Convertlet that handles the BigQuery {@code TIMESTAMP_SUB} function. */
specifier|private
specifier|static
class|class
name|TimestampSubConvertlet
...
parameter_list|)
block|{
comment|// TIMESTAMP_SUB(timestamp, interval)
comment|//  => timestamp - count * INTERVAL '1' UNIT
specifier|final
name|RexBuilder
name|rexBuilder
...
condition|)
block|{
comment|//Fractional second units are converted to seconds using their associated multiplier.
case|case
name|MICROSECOND
case|:
case|case
...
condition|)
block|{
comment|// Custom time frames have a different path. They are kept as names, and
comment|// then handled by Java functions.
specifier|final
name|RexLiteral
name|timeFrameName
init|=
