...
argument_list|,
operator|new
name|JsonOperatorValueExprConvertlet
argument_list|(
literal|0
argument_list|)
argument_list|)
...
argument_list|)
expr_stmt|;
comment|// Convert json_value('{"foo":"bar"}', 'lax $.foo', returning varchar(2000))
comment|// to cast(json_value('{"foo":"bar"}', 'lax $.foo') as varchar(2000))
name|registerOp
argument_list|(
name|SqlStdOperatorTable
operator|.
...
block|}
block|}
comment|/** Convertlet that adds implicit calls to    * {@link org.apache.calcite.sql.fun.SqlJsonValueExpressionOperator}    * for a series fo JSON operators.    *    *<p> A call to JSON operator sometimes has a specific expression    * argument (it's always the first argument of the call), which implies    * a "FORMAT JSON" postfix when the argument is not a explicit call to the    * {@link org.apache.calcite.sql.fun.SqlJsonValueExpressionOperator}. */
specifier|private
class|class
name|JsonOperatorValueExprConvertlet
implements|implements
...
name|int
index|[]
name|implicitJsonValueExprOrdinals
decl_stmt|;
specifier|private
name|JsonOperatorValueExprConvertlet
parameter_list|(
...
parameter_list|(
name|int
name|implicitJsonValueExprOrdinal
parameter_list|)
block|{
name|this
argument_list|(
...
name|JSON_VALUE_EXPRESSION
decl_stmt|;
comment|// Validate the original operand using the new operator. JSON value expression
comment|// accepts only a character input.
comment|//
comment|// Once CALCITE-2869 is fixed, we can validate the original operand during SQL
comment|// validation using SqlTypeName.JSON.
name|RelDataType
name|validateOperandType
init|=
name|cx
operator|.
name|getValidator
...
condition|)
block|{
comment|// must be character type
throw|throw
name|cx
operator|.
name|getValidator
