...
return|;
block|}
comment|/** Decorrelates a query.    *    *<p>This is the main entry point to {@code RelDecorrelator}.    *    * @param rootRel           Root node of the query    * @param relBuilder        Builder for relational expressions    *    * @return Equivalent query with all    * {@link org.apache.calcite.rel.core.Correlate} instances removed    */
specifier|public
specifier|static
name|RelNode
name|decorrelateQuery
...
name|debug
argument_list|(
literal|"Plan before extracting correlated computations:\n"
operator|+
name|RelOptUtil
operator|.
name|toString
...
argument_list|)
expr_stmt|;
comment|// Necessary to update cm (CorrelMap) since CorrelateProjectExtractor above may modify the plan
name|this
operator|.
name|cm
operator|=
...
name|debug
argument_list|(
literal|"Plan after extracting correlated computations:\n"
operator|+
name|RelOptUtil
operator|.
name|toString
...
name|r
decl_stmt|;
comment|// aggregate outputs mapping: group keys and aggregates
specifier|final
name|Map
argument_list|<
name|Integer
...
continue|continue;
block|}
comment|// add mapping of group keys.
name|outputMap
operator|.
name|put
argument_list|(
...
parameter_list|()
lambda|->
literal|"mapNewInputToProjOutputs.get("
operator|+
name|entry
operator|.
name|getValue
...
argument_list|)
expr_stmt|;
comment|// Shift the outputs whose index equals with or bigger than the added index
comment|// with 1 offset.
name|shiftMapping
argument_list|(
name|outputMap
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Then add the constant key mapping.
name|outputMap
operator|.
name|put
argument_list|(
...
name|r
decl_stmt|;
comment|// Can not decorrelate if the sort has per-correlate-key attributes like
comment|// offset or fetch limit, because these attributes scope would change to
comment|// global after decorrelation. They should take effect within the scope
comment|// of the correlation key actually.
if|if
condition|(
name|sort
operator|.
...
parameter_list|()
lambda|->
literal|"mapNewInputToOutputs.get("
operator|+
name|newInput
operator|+
literal|")"
...
parameter_list|()
lambda|->
literal|"mapNewInputToNewOffset.get("
operator|+
name|newInput
operator|+
literal|")"
...
parameter_list|()
lambda|->
literal|"r.getInput(0) is null for "
operator|+
name|r
argument_list|)
return|;
...
block|{
return|return
name|ImmutableRemoveSingleAggregateRuleConfig
operator|.
name|builder
argument_list|()
operator|.
...
block|{
return|return
name|ImmutableRemoveCorrelationForScalarProjectRuleConfig
operator|.
name|builder
argument_list|()
operator|.
...
block|{
return|return
name|ImmutableRemoveCorrelationForScalarAggregateRuleConfig
operator|.
name|builder
argument_list|()
operator|.
...
block|{
return|return
name|ImmutableAdjustProjectForCountAggregateRuleConfig
operator|.
name|builder
argument_list|()
operator|.
...
block|}
block|}
comment|/** Base configuration for rules that are non-static in a RelDecorrelator. */
specifier|public
interface|interface
name|Config
extends|extends
...
parameter_list|()
function_decl|;
comment|/** Sets {@link #decorrelator}. */
name|Config
name|withDecorrelator
parameter_list|(
name|RelDecorrelator
