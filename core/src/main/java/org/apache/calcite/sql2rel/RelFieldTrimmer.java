...
parameter_list|)
block|{
comment|// We don't know how to trim this kind of relational expression
name|Util
operator|.
name|discard
argument_list|(
...
return|;
block|}
comment|// We don't know how to trim this RelNode, but we can try to trim inside its inputs
name|List
argument_list|<
name|RelNode
argument_list|>
...
name|IllegalArgumentException
argument_list|(
literal|"Expected identity mapping after processing RelNode "
operator|+
name|input
operator|+
literal|"; but got "
...
return|;
block|}
comment|/**    * Variant of {@link #trimFields(RelNode, ImmutableBitSet, Set)} for    * {@link org.apache.calcite.rel.logical.LogicalCalc}.    */
specifier|public
name|TrimResult
name|trimFields
parameter_list|(
...
return|;
block|}
comment|/** Creates a project with a dummy column, to protect the parts of the system    * that cannot handle a relational expression with no columns.    *    * @param fieldCount Number of fields in the original relational expression    * @param input Trimmed input    * @return Dummy project    */
specifier|protected
name|TrimResult
name|dummyProject
parameter_list|(
...
return|;
block|}
comment|/** Creates a project with a dummy column, to protect the parts of the system    * that cannot handle a relational expression with no columns.    *    * @param fieldCount Number of fields in the original relational expression    * @param input Trimmed input    * @param originalRelNode Source RelNode for hint propagation (or null if no propagation needed)    * @return Dummy project    */
specifier|protected
name|TrimResult
name|dummyProject
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// We use the fields used by the consumer, plus any fields used as exchange
comment|// keys.
specifier|final
name|ImmutableBitSet
operator|.
...
argument_list|()
decl_stmt|;
comment|// We use the fields used by the consumer, plus any fields used as sortExchange
comment|// keys.
specifier|final
name|ImmutableBitSet
operator|.
...
condition|)
block|{
comment|// Add a dummy call if all the column fields have been trimmed
name|mapping
operator|=
name|Mappings
operator|.
