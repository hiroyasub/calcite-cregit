...
name|config
operator|.
name|isAddJsonTypeOperatorEnabled
argument_list|()
condition|)
block|{
name|result
...
argument_list|(
operator|new
name|NestedJsonFunctionRelRewriter
argument_list|()
argument_list|)
expr_stmt|;
block|}
...
return|return;
block|}
name|substituteSubQueryOfSetSemanticsInputTable
argument_list|(
name|bb
argument_list|,
name|subQuery
...
parameter_list|()
lambda|->
literal|"input RelNode is null for query "
operator|+
name|query
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// For set semantics table, distribution is singleton if does not specify
...
comment|// This is used when converting window table functions:
comment|//
comment|// select * from table(tumble(table emps, descriptor(deptno),
comment|// interval '3' DAY))
comment|//
name|bb
operator|.
name|cursors
...
return|;
block|}
comment|/**    * Note: The ORDER BY clause for input table parameter differs from the    * ORDER BY clause in some other contexts in that only columns may be sorted    * (not arbitrary expressions).    *    * @param bb              Scope within which to resolve identifiers    * @param orderList       Order by clause, may be null    * @return ordering of input table    */
specifier|private
name|RelCollation
name|buildCollation
parameter_list|(
...
name|SqlTypeUtil
operator|.
name|keepSourceTypeAndTargetNullability
argument_list|(
name|targetRowType
argument_list|,
name|listType
...
decl_stmt|;
comment|// ORDER BY
comment|// TODO combine with buildCollation method after support NULLS_FIRST/NULLS_LAST
specifier|final
name|SqlNodeList
name|orderList
init|=
...
name|Project
assert|;
comment|// correlation variables have been normalized in p.r, we should use expressions
comment|// in p.r instead of the original exprs
name|Project
name|project1
init|=
operator|(
...
argument_list|)
decl_stmt|;
comment|// Convert qualify SqlNode to a RexNode
name|replaceSubQueries
argument_list|(
name|bb
argument_list|,
...
try|try
block|{
comment|// Set the root to the input of the project,
comment|// since QUALIFY might have an expression in the OVER clause
comment|// that references a column not in the SELECT.
name|bb
operator|.
name|setRoot
argument_list|(
...
expr_stmt|;
block|}
comment|// Check to see if the qualify expression has a referenced expression and
comment|// do some referencing accordingly
specifier|final
name|RexNode
name|qualifyWithReferencesRexNode
init|=
name|qualifyRexNode
operator|.
name|accept
...
argument_list|)
decl_stmt|;
comment|// Create a Project with the QUALIFY expression
if|if
condition|(
name|qualifyWithReferencesRexNode
operator|.
...
condition|)
block|{
comment|// The QUALIFY expression does not depend on any references like so:
comment|//
comment|//  SELECT A, B
comment|//  FROM tbl
comment|//  QUALIFY WINDOW(C) = 1
comment|//
comment|// Meaning we should generate a plan like:
comment|//  Project(A, B, WINDOW(C) = 1 as QualifyExpression)
comment|//    TableScan(tbl)
comment|//
name|relBuilder
operator|.
name|push
...
block|{
comment|// The QUALIFY expression depended on a reference meaning
comment|// we need to introduce an extra project like so:
comment|//
comment|//  SELECT A, B, WINDOW(C) as window_val
comment|//  FROM tbl
comment|//  QUALIFY window_val = 1
comment|//
comment|// Meaning we should generate a plan like:
comment|//
comment|//  Project($0, $1, $2, =($2, 1) as QualifyExpression)
comment|//    Project(A, B, WINDOW(C) as window_val)
comment|//      TableScan(tbl)
comment|//
comment|// This is a very specific application of Common Subexpression Elimination
comment|// (CSE), since the window value pops up twice.
name|relBuilder
operator|.
name|push
argument_list|(
...
expr_stmt|;
block|}
comment|// Filter on that extra column
name|relBuilder
operator|.
name|filter
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Remove that extra column from the projection
name|relBuilder
operator|.
name|project
argument_list|(
...
expr_stmt|;
block|}
comment|/** Eliminates a common sub-expression by looking for a {@link RexNode}    * in the expressions of a {@link Project}; if found, returns a refIndex    * instead of the raw node. */
specifier|private
specifier|static
specifier|final
class|class
...
expr_stmt|;
block|}
comment|/**      * Returns an expression with which to reference a from-list item;      * throws if not found.      *      * @param qualified The alias of the FROM item      * @return a {@link RexFieldAccess} or {@link RexRangeRef}, never null      */
name|Pair
argument_list|<
name|RexNode
argument_list|,
...
name|AssertionError
argument_list|(
literal|"no unique expression found for "
operator|+
name|qualified
operator|+
literal|"; count is "
...
return|;
block|}
comment|// Reference comparing does not work in case when select list has column which refers
comment|// to the column inside `GROUP BY` clause.
comment|// For example: SELECT deptno IN (1,2) FROM emp.deptno GROUP BY deptno IN (1,2);
if|if
condition|(
name|exprClause
operator|==
...
block|}
block|}
comment|// Only used by deprecated method "convertSortExpression", and will be
comment|// removed with that method.
specifier|private
name|RexFieldCollation
name|sortToRexFieldCollation
parameter_list|(
...
parameter_list|)
function_decl|;
comment|/**      * Whether add {@link SqlStdOperatorTable#JSON_TYPE_OPERATOR} for between json functions.      */
annotation|@
name|Value
operator|.
name|Default
...
return|;
block|}
comment|/** Sets {@link #isAddJsonTypeOperatorEnabled()}. */
name|Config
name|withAddJsonTypeOperatorEnabled
parameter_list|(
name|boolean
name|addJsonTypeOperatorEnabled
parameter_list|)
function_decl|;
block|}
comment|/**    * Used to find nested json functions, and add {@link SqlStdOperatorTable#JSON_TYPE_OPERATOR}    * to nested json output.    */
specifier|private
class|class
name|NestedJsonFunctionRelRewriter
extends|extends
...
name|Integer
argument_list|>
name|requiredJsonFieldsFromParent
init|=
name|stack
operator|.
name|size
...
literal|0
condition|?
name|requiredJsonOutputFromParent
argument_list|(
name|stack
operator|.
name|getLast
...
block|}
block|}
comment|/**    * Used to rewrite json functions which is nested.    */
specifier|private
class|class
name|JsonFunctionRexRewriter
extends|extends
