...
argument_list|)
decl_stmt|;
comment|// Attach the hints to the first project we found from the root node.
name|bb
operator|.
name|setRoot
argument_list|(
...
condition|)
block|{
comment|// the WHERE in a WITHIN_GROUP must be tracked too so we can call replaceSubQueries on it.
comment|// see https://issues.apache.org/jira/browse/CALCITE-1910
specifier|final
name|SqlNode
name|aggCall
...
block|}
block|}
comment|/**    * A {@code RelShuttle} which propagates all the hints of relational expression to    * their children nodes.    *    *<p>Given a plan:    *    *<blockquote><pre>    * Filter Hint1    * +- Join    *    +- Scan    *    +- Project Hint2    *       +- Scan    *</pre></blockquote>    *    *<p>Every hint has a {@code inheritPath} (integers list) which records its propagate path,    * number `0` represents the hint is propagated from the first(left) child, number `1`    * represents the hint is propagated from the second(right) child, so the plan would have    * hints path as follows:    *    *<ol>    *<li>Filter would have hints {Hint1[]}</li>    *<li>Join would have hints {Hint1[0]}</li>    *<li>Scan would have hints {Hint1[0, 0]}</li>    *<li>Project would have hints {Hint1[0,1], Hint2}</li>    *<li>Scan2 would have hints {[Hint1[0, 1, 0], Hint2[0]}</li>    *</ol>    */
specifier|private
specifier|static
class|class
name|RelHintPropagateShuttle
...
begin_comment
unit|}
comment|/** Creates a builder for a {@link Config}. */
end_comment
...
begin_comment
comment|/**    * Interface to define the configuration for a SqlToRelConverter.    * Provides methods to set each configuration option.    *    * @see ConfigBuilder    * @see SqlToRelConverter#configBuilder()    */
end_comment
