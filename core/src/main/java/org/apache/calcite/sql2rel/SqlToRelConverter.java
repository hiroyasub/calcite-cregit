...
block|}
comment|//~ Methods ----------------------------------------------------------------
comment|// helper for constructor
specifier|private
specifier|static
name|RelOptCluster
name|createCluster
...
expr_stmt|;
block|}
comment|/**    * Controls whether instances of    * {@link org.apache.calcite.rel.logical.LogicalValues} are generated. These    * may not be supported by all physical implementations. To have any effect,    * this must be called before any convert method.    *    * @param enabled true to allow LogicalValues to be generated (the default);    *                false to force substitution of Project+OneRow instead    */
specifier|public
name|void
...
expr_stmt|;
block|}
comment|// TODO: This conversion is only valid in the WHERE clause
if|if
condition|(
name|logic
operator|==
...
block|}
block|}
comment|/**    * Returns an expression for matching columns of a USING clause or inferred    * from NATURAL JOIN. "a JOIN b USING (x, y)" becomes "a.x = b.x AND a.y =    * b.y". Returns null if the column list is empty.    *    * @param leftRel  Left input to the join    * @param rightRel Right input to the join    * @param nameList List of column names to join on    * @return Expression to match columns from name list, or true if name list    * is empty    */
specifier|private
name|RexNode
name|convertUsing
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Generate NULL values for rolled-up not-null fields.
specifier|final
name|Aggregate
name|aggregate
init|=
