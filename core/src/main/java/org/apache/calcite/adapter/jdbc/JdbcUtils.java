...
block|}
block|}
comment|/** Ensures that if two data sources have the same definition, they will use    * the same object.    *    *<p>This in turn makes it easier to cache    * {@link org.eigenbase.sql.SqlDialect} objects. Otherwise, each time we    * see a new data source, we have to open a connection to find out what    * database product and version it is. */
specifier|public
specifier|static
class|class
name|DataSourcePool
