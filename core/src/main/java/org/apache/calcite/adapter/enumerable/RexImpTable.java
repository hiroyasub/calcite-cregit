...
begin_comment
comment|/**  * Contains implementations of Rex operators as Java code.  *  *<p>Immutable.  */
end_comment
...
expr_stmt|;
block|}
comment|/** Holds intermediate state from which a RexImpTable can be constructed. */
specifier|private
specifier|static
class|class
name|Builder
...
argument_list|()
decl_stmt|;
comment|/** Populates this Builder with implementors for all Calcite built-in and      * library operators. */
name|Builder
name|populate
parameter_list|()
block|{
...
return|;
block|}
comment|/** Second step of population. The {@code populate} method grew too large,      * and we factored this out. Feel free to decompose further. */
name|Builder
name|populate2
parameter_list|()
block|{
...
name|BuiltInMethod
operator|.
name|CUSTOM_TIMESTAMP_FLOOR
operator|.
name|method
argument_list|,
name|BuiltInMethod
operator|.
name|CUSTOM_DATE_FLOOR
operator|.
name|method
argument_list|)
argument_list|)
...
name|BuiltInMethod
operator|.
name|CUSTOM_TIMESTAMP_CEIL
operator|.
name|method
argument_list|,
name|BuiltInMethod
operator|.
name|CUSTOM_DATE_CEIL
operator|.
name|method
argument_list|)
argument_list|)
...
argument_list|,
operator|new
name|TimestampAddImplementor
argument_list|(
literal|"timestampAdd"
argument_list|,
name|BuiltInMethod
operator|.
name|CUSTOM_TIMESTAMP_ADD
operator|.
name|method
argument_list|,
name|BuiltInMethod
operator|.
name|CUSTOM_DATE_ADD
operator|.
name|method
argument_list|)
argument_list|)
...
argument_list|,
operator|new
name|TimestampDiffImplementor
argument_list|(
literal|"timestampDiff"
argument_list|,
name|BuiltInMethod
operator|.
name|CUSTOM_TIMESTAMP_DIFF
operator|.
name|method
argument_list|,
name|BuiltInMethod
operator|.
name|CUSTOM_DATE_DIFF
operator|.
name|method
argument_list|)
argument_list|)
...
argument_list|)
expr_stmt|;
comment|// Datetime formatting methods
specifier|final
name|FormatDatetimeImplementor
name|datetimeFormatImpl
init|=
operator|new
name|FormatDatetimeImplementor
argument_list|()
...
argument_list|,
operator|new
name|QuantifyCollectionImplementor
argument_list|(
name|binaryOperator
argument_list|,
name|binaryImplementor
...
block|}
block|}
comment|/** Implementor for the {@code ARG_MIN} and {@code ARG_MAX} aggregate    * functions. */
specifier|static
class|class
name|ArgMinMaxImplementor
extends|extends
...
operator|)
decl_stmt|;
comment|//acc[1] = isMin ? {max value} : {min value};
name|reset
operator|.
name|currentBlock
argument_list|()
...
block|}
block|}
comment|/** Implementor for the {@code LAST_DAY} function. */
specifier|private
specifier|static
class|class
name|LastDayImplementor
...
specifier|final
name|Method
name|customTimestampMethod
decl_stmt|;
specifier|final
name|Method
name|customDateMethod
decl_stmt|;
name|FloorImplementor
parameter_list|(
name|String
...
block|}
block|}
comment|/** Implementor for the {@code TIMESTAMPADD} function. */
specifier|private
specifier|static
class|class
name|TimestampAddImplementor
...
block|}
block|}
comment|/** Implementor for the {@code TIMESTAMPDIFF} function. */
specifier|private
specifier|static
class|class
name|TimestampDiffImplementor
...
block|}
block|}
comment|/**    * Implementor for the {@code FORMAT_TIMESTAMP, FORMAT_DATE, FORMAT_TIME} and    * {@code FORMAT_DATETIME} functions.    */
specifier|private
specifier|static
class|class
name|FormatDatetimeImplementor
...
name|RexCallImplementor
block|{
comment|/** Variable name should be meaningful. It helps us debug issues. */
specifier|final
name|String
name|variableName
decl_stmt|;
...
block|}
block|}
comment|/**    * Implements    *<a href="https://www.postgresql.org/docs/current/functions-comparisons.html#id-1.5.8.30.16">    * ANY/SOME</a> and    *<a href="https://www.postgresql.org/docs/current/functions-comparisons.html#id-1.5.8.30.17">ALL</a>    * operators when the argument is an array or multiset expression.    */
specifier|private
specifier|static
class|class
name|QuantifyCollectionImplementor
...
argument_list|)
decl_stmt|;
comment|// If the array expression yields a null array, the result of SOME|ALL will be null
if|if
condition|(
name|rightComponentType
operator|.
...
return|;
block|}
comment|// The expression generated by this method will look as follows:
comment|// final T _quantify_left_value =<left_value>
comment|//<Function1|Predicate1> lambda =
comment|//    new org.apache.calcite.linq4j.function.<Function1|Predicate1>() {
comment|//        public Boolean apply(T el) {
comment|//          return<binaryImplementor code>(_quantify_left_value, el);
comment|//        }
comment|//    }
comment|// If the lambda returns java.lang.Boolean then the lambda can return null.
comment|// In this case nullableExists or nullableSome should be used:
comment|// return org.apache.calcite.runtime.SqlFunctions.<nullableExists|nullableSome>(_list, lambda)
comment|// otherwise:
comment|// return org.apache.calcite.linq4j.function.Functions.<exists|all>(_list, lambda)
name|BlockBuilder
name|lambdaBuilder
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// left should have final modifier otherwise it can not be passed to lambda
name|translator
operator|.
name|getBlockBuilder
argument_list|()
...
operator|.
name|method
comment|// if the array contains any null elements and no true comparison result is obtained,
comment|// the result of SOME will be null, not false.
else|:
name|BuiltInMethod
operator|.
name|COLLECTION_NULLABLE_EXISTS
operator|.
name|method
return|;
case|case
...
operator|.
name|method
comment|// if the array contains any null elements and no false comparison result is obtained,
comment|// the result of ALL will be null, not true.
else|:
name|BuiltInMethod
operator|.
name|COLLECTION_NULLABLE_ALL
operator|.
name|method
return|;
default|default:
...
name|IllegalArgumentException
argument_list|(
literal|"Unknown quantify operator "
operator|+
name|kind
operator|+
literal|". Only support SOME, ALL."
argument_list|)
throw|;
block|}
block|}
