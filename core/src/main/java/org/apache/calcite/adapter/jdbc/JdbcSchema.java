...
name|parentSchema
argument_list|,
literal|"parentSchema must not be null for JdbcSchema"
argument_list|)
expr_stmt|;
return|return
name|Schemas
...
return|;
block|}
comment|/** Returns a pair of (catalog, schema) for the current connection. */
specifier|private
name|Pair
argument_list|<
annotation|@
...
condition|)
block|{
comment|// From JDBC 4.1, catalog and schema can be retrieved from the connection
comment|// object, hence try to get it from there if it was not specified by user
name|catalog
operator|=
name|connection
operator|.
...
literal|null
expr_stmt|;
comment|// PostgreSQL returns useless "" sometimes
block|}
block|}
if|if
condition|(
...
name|sql
init|=
literal|"select current_database(), current_schema()"
decl_stmt|;
try|try
init|(
name|Statement
