...
decl_stmt|;
name|Expression
name|lambdaForStructWithSingleItem
init|=
literal|null
decl_stmt|;
for|for
...
condition|)
block|{
comment|// Solves CALCITE-4063: if we are processing a single field, which is a struct with a
comment|// single item inside, and no ordinality; the result must be a scalar, hence use a
comment|// special lambda that does not return lists, but the (single) items within those lists
name|lambdaForStructWithSingleItem
operator|=
name|Expressions
operator|.
