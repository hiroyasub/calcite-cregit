...
begin_comment
comment|/** Returns an expression that yields the function object whose method    * we are about to call.    *    *<p>It might be 'new MyFunction()', but it also might be a reference    * to a static field 'F', defined by    * 'static final MyFunction F = new MyFunction()'.    * In future, we might look for and call a constructor that takes extra    * context, such as the values of arguments that are literals, which might    * allow the function to do some computation at class-load time. */
end_comment
