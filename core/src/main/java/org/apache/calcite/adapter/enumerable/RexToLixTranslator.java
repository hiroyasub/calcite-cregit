...
specifier|private
name|Expression
name|handleNullUnboxingIfNecessary
parameter_list|(
name|Expression
name|input
parameter_list|,
...
comment|// unboxing which may occur in the handleNull method below.
return|return
name|input
return|;
block|}
...
return|;
block|}
comment|/** Adapts an expression with "normal" result to one that adheres to    * this particular policy. Wraps the result expression into a new    * parameter if need be.    *    * @param input Expression    * @param nullAs If false, if expression is definitely not null at    *   runtime. Therefore we can optimize. For example, we can cast to int    *   using x.intValue().    * @return Translated expression    */
specifier|public
name|Expression
name|handleNull
parameter_list|(
