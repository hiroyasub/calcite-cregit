...
return|;
block|}
comment|// EnumerableCorrelate traits passdown shall only pass through collation to left input.
comment|// This is because for EnumerableCorrelate always uses left input as the outer loop,
...
control|)
block|{
comment|// If field collation belongs to right input: bail out.
if|if
condition|(
name|relFieldCollation
operator|.
...
parameter_list|)
block|{
comment|// should only derive traits (limited to collation for now) from left input.
assert|assert
name|childId
operator|==
literal|0
