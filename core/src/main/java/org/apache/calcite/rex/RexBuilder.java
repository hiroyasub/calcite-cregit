...
argument_list|)
decl_stmt|;
comment|/** The smallest valid {@code int} value, as a {@link BigDecimal}. */
specifier|private
specifier|static
specifier|final
name|BigDecimal
...
argument_list|)
decl_stmt|;
comment|/** The largest valid {@code int} value, as a {@link BigDecimal}. */
specifier|private
specifier|static
specifier|final
name|BigDecimal
...
name|indicator
argument_list|,
literal|"indicator is deprecated, use GROUPING function instead"
argument_list|)
expr_stmt|;
return|return
name|addAggCall
...
return|;
block|}
comment|/**    * Creates a call to the CAST operator, expanding if possible, and optionally    * also preserving nullability, and optionally in safe mode.    *    *<p>Tries to expand the cast, and therefore the result may be something    * other than a {@link RexCall} to the CAST operator, such as a    * {@link RexLiteral}.    *    * @param type Type to cast to    * @param exp  Expression being cast    * @param matchNullability Whether to ensure the result has the same    * nullability as {@code type}    * @param safe Whether to return NULL if cast fails    * @return Call to CAST operator    */
specifier|public
name|RexNode
name|makeCast
parameter_list|(
...
return|;
block|}
comment|/** Returns the lowest granularity unit for the given unit.    * YEAR and MONTH intervals are stored as months;    * HOUR, MINUTE, SECOND intervals are stored as milliseconds. */
specifier|protected
specifier|static
name|TimeUnit
name|baseUnit
...
literal|1
case|:
comment|// E.g. multiplyDivide(e, 1000, 10) ==> e * 100
return|return
name|makeCall
argument_list|(
name|SqlStdOperatorTable
...
literal|1
case|:
comment|// E.g. multiplyDivide(e, 10, 1000) ==> e / 100
return|return
name|makeCall
argument_list|(
name|SqlStdOperatorTable
...
return|;
block|}
comment|/**    * Creates a call to CAST or SAFE_CAST operator.    *    * @param type Type to cast to    * @param exp  Expression being cast    * @param safe Whether to return NULL if cast fails    * @return Call to CAST operator    */
specifier|public
name|RexNode
name|makeAbstractCast
parameter_list|(
...
return|;
block|}
comment|/**    * Makes a cast of a value to NOT NULL;    * no-op if the type already has NOT NULL.    */
specifier|public
name|RexNode
name|makeNotNull
parameter_list|(
...
return|;
block|}
comment|/**    * Create a reference to local variable.    *    * @param type Type of variable    * @param i    Ordinal of variable    * @return  Reference to local variable    */
specifier|public
name|RexLocalRef
name|makeLocalRef
parameter_list|(
...
literal|null
operator|:
literal|"type.getCharset() must not be null"
assert|;
assert|assert
name|type
operator|.
...
literal|null
operator|:
literal|"type.getCollation() must not be null"
assert|;
name|o
operator|=
operator|new
...
return|;
block|}
comment|/**    * Creates a search argument literal.    */
specifier|public
name|RexLiteral
...
return|;
block|}
comment|/**    * Creates a character string literal with type CHAR.    *    * @param value       String value in bytes    * @param charsetName SQL-level charset name    * @param collation   Sql collation    * @return String     literal    */
specifier|protected
name|RexLiteral
name|makePreciseStringLiteral
parameter_list|(
...
return|;
block|}
comment|/**    * Creates a Time with local time-zone literal.    */
specifier|public
name|RexLiteral
...
return|;
block|}
comment|/**    * Creates a Timestamp with local time-zone literal.    */
specifier|public
name|RexLiteral
...
return|;
block|}
comment|/**    * Creates a literal representing an interval type, for example    * {@code YEAR TO MONTH} or {@code DOW}.    */
specifier|public
name|RexLiteral
...
return|;
block|}
comment|/**    * Creates a literal representing an interval value, for example    * {@code INTERVAL '3-7' YEAR TO MONTH}.    */
specifier|public
name|RexLiteral
name|makeIntervalLiteral
parameter_list|(
...
return|;
block|}
comment|/** Creates a {@link RexNode} representation a SQL "arg IN (point, ...)"    * expression.    *    *<p>If all of the expressions are literals, creates a call {@link Sarg}    * literal, "SEARCH(arg, SARG([point0..point0], [point1..point1], ...)";    * otherwise creates a disjunction, "arg = point0 OR arg = point1 OR ...". */
specifier|public
name|RexNode
name|makeIn
parameter_list|(
...
parameter_list|()
lambda|->
literal|"Can't find leastRestrictive type for SARG among "
operator|+
name|types
argument_list|)
decl_stmt|;
...
return|;
block|}
comment|/** Converts a list of expressions to a search argument, or returns null if    * not possible. */
annotation|@
name|SuppressWarnings
argument_list|(
block|{
...
condition|)
block|{
comment|// Cannot convert an empty list to a Sarg (by this interface, at least)
comment|// because we use the type of the first element.
return|return
literal|null
return|;
block|}
...
name|IllegalArgumentException
argument_list|(
literal|"Cannot create literal for type '"
operator|+
name|sqlTypeName
operator|+
literal|"'"
...
block|}
block|}
comment|/** Converts the type of a value to comply with    * {@link org.apache.calcite.rex.RexLiteral#valueMatchesType}.    *    *<p>Returns null if and only if {@code o} is null. */
specifier|private
specifier|static
annotation|@
name|PolyNull
...
name|ROOT
argument_list|,
literal|"%s is not compatible with %s, try to use makeExactLiteral"
argument_list|,
name|o
operator|.
name|getClass
...
name|type
operator|+
literal|".getCharset() must not be null"
assert|;
return|return
operator|new
name|NlsString
