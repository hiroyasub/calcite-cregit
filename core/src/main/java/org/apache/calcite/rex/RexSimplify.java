...
return|;
block|}
comment|/** Applies IS NOT FALSE to an expression. */
name|RexNode
name|isNotFalse
parameter_list|(
name|RexNode
...
condition|?
name|e
comment|// would "CAST(e AS BOOLEAN NOT NULL)" better?
else|:
name|rexBuilder
operator|.
name|makeCall
...
return|;
block|}
comment|/** Applies IS NOT TRUE to an expression. */
name|RexNode
name|isNotTrue
parameter_list|(
name|RexNode
...
return|;
block|}
comment|/** Applies IS TRUE to an expression. */
name|RexNode
name|isTrue
parameter_list|(
name|RexNode
...
return|;
block|}
comment|/** Applies IS FALSE to an expression. */
name|RexNode
name|isFalse
parameter_list|(
name|RexNode
...
condition|)
block|{
comment|// we only support simplifying numeric types.
return|return
name|simplifyGenericNode
argument_list|(
name|e
...
argument_list|(
literal|"Unsupported arithmeitc operation "
operator|+
name|e
operator|.
name|getKind
...
condition|)
block|{
comment|// return the other operand.
name|RexNode
name|other
init|=
name|e
...
condition|)
block|{
comment|// "x LIKE '%'" simplifies to "x = x"
specifier|final
name|RexNode
name|x
init|=
...
name|SEARCH
case|:
comment|// NOT SEARCH(x, Sarg[(-inf, 10) OR NULL) ==> SEARCH(x, Sarg[[10, +inf)])
specifier|final
name|RexCall
name|call2
init|=
...
comment|// but not interfere with the normal simplification recursion
name|List
argument_list|<
name|CaseBranch
argument_list|>
...
name|AssertionError
argument_list|(
literal|"result mismatch (unknown as "
operator|+
name|unknownAs
operator|+
literal|"): when applied to "
operator|+
name|map
operator|+
literal|",\n"
...
argument_list|)
decl_stmt|;
comment|// Now we've strengthened the Sarg, try to simplify again
return|return
name|simplifySearch
argument_list|(
name|call
...
condition|)
block|{
comment|// Expand "SEARCH(x, Sarg([point])" to "x = point"
comment|// and "SEARCH(x, Sarg([])" to "false"
return|return
name|RexUtil
operator|.
name|expandSearch
...
condition|)
block|{
comment|// Cannot include expressions 'x> NULL' in a Sarg. Comparing to a NULL
comment|// literal is a daft thing to do, because it always returns UNKNOWN. It
comment|// is better handled by other simplifications.
return|return
literal|false
return|;
block|}
...
block|}
block|}
comment|/** Equivalent to a {@link RexLiteral} whose value is a {@link Sarg},    * but mutable, so that the Sarg can be expanded as {@link SargCollector}    * traverses a list of OR or AND terms.    *    *<p>The {@link SargCollector#fix} method converts it to an immutable    * literal.    *    *<p>The {@link #nullAs} field will become {@link Sarg#nullAs}, as follows:    *    *<ul>    *<li>If there is at least one term that returns TRUE when the argument    * is NULL, then the overall value will be TRUE; failing that,    *<li>if there is at least one term that returns UNKNOWN when the argument    * is NULL, then the overall value will be UNKNOWN; failing that,    *<li>the value will be FALSE.    *</ul>    *    *<p>This is analogous to the behavior of OR in three-valued logic:    * {@code TRUE OR UNKNOWN OR FALSE} returns {@code TRUE};    * {@code UNKNOWN OR FALSE OR UNKNOWN} returns {@code UNKNOWN};    * {@code FALSE OR FALSE} returns {@code FALSE}. */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"BetaApi"
