...
expr_stmt|;
block|}
comment|// Check for equality on different constants. If the same ref or
comment|// CAST(ref) is equal to different constants, this condition cannot be
comment|// satisfied, and hence it can be evaluated to FALSE.
if|if
condition|(
name|term
operator|.
...
comment|// But as comparison is string-based and thus operands order-dependent,
comment|// we should also add the inverted negated term: 5>= a.
comment|// Observe that for creating the inverted term we invert the list of
comment|// operands.
name|RexNode
name|negatedTerm
init|=
