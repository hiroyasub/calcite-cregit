...
begin_comment
comment|/**  * Planner rule that translates a distinct  * {@link org.apache.calcite.rel.core.Intersect}  * (<code>all</code> =<code>false</code>)  * into a group of operators composed of  * {@link org.apache.calcite.rel.core.Union},  * {@link org.apache.calcite.rel.core.Aggregate}, etc.  *  *<p> Rewrite: (GB-Union All-GB)-GB-UDTF (on all attributes)  *  *<h2>Example</h2>  *  *<p>Query:<code>R1 Intersect All R2</code>  *  *<p><code>R3 = GB(R1 on all attributes, count(*) as c)<br>  *   union all<br>  *   GB(R2 on all attributes, count(*) as c)</code>  *  *<p><code>R4 = GB(R3 on all attributes, count(c) as cnt, min(c) as m)</code>  *  *<p>Note that we do not need<code>min(c)</code> in intersect distinct.  *  *<p><code>R5 = Filter(cnt == #branch)</code>  *  *<p>If it is intersect all then  *  *<p><code>R6 = UDTF (R5) which will explode the tuples based on min(c)<br>  * R7 = Project(R6 on all attributes)</code>  *  *<p>Else  *  *<p><code>R6 = Proj(R5 on all attributes)</code>  *  * @see org.apache.calcite.rel.rules.UnionToDistinctRule  * @see CoreRules#INTERSECT_TO_DISTINCT  */
end_comment
...
name|DEFAULT
init|=
name|ImmutableIntersectToDistinctRule
operator|.
name|Config
operator|.
name|of
