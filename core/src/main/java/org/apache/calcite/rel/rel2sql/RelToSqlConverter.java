...
return|;
block|}
comment|/** Returns whether this join should be unparsed as a {@link JoinType#COMMA}.    *    *<p>Comma-join is one possible syntax for {@code CROSS JOIN ... ON TRUE},    * supported on most but not all databases    * (see {@link SqlDialect#emulateJoinTypeForCrossJoin()}).    *    *<p>For example, the following queries are equivalent:    *    *<pre>{@code    * // Comma join    * SELECT *    * FROM Emp, Dept    *    * // Cross join    * SELECT *    * FROM Emp CROSS JOIN Dept    *    * // Inner join    * SELECT *    * FROM Emp INNER JOIN Dept ON TRUE    * }</pre>    *    *<p>Examples:    *<ul>    *<li>{@code FROM (x CROSS JOIN y ON TRUE) CROSS JOIN z ON TRUE}    *   is a comma join, because all joins are comma joins;    *<li>{@code FROM (x CROSS JOIN y ON TRUE) CROSS JOIN z ON TRUE}    *   would not be a comma join when run on Apache Spark, because Spark does    *   not support comma join;    *<li>{@code FROM (x CROSS JOIN y ON TRUE) LEFT JOIN z ON TRUE}    *   is not comma join because one of the joins is not INNER;    *<li>{@code FROM (x CROSS JOIN y ON c) CROSS JOIN z ON TRUE}    *   is not a comma join because one of the joins is ON TRUE.    *</ul>    */
specifier|private
name|boolean
name|isCommaJoin
parameter_list|(
...
return|;
block|}
comment|// Find the topmost enclosing Join. For example, if the tree is
comment|//   Join((Join(a, b), Join(c, Join(d, e)))
comment|// we get the same topJoin for a, b, c, d and e. Join. Stack is never empty,
comment|// and frame.r on the first iteration is always "join".
assert|assert
operator|!
name|stack
operator|.
...
argument_list|)
decl_stmt|;
comment|// Flatten the join tree, using a breadth-first algorithm.
comment|// After flattening, the list contains all of the joins that will make up
...
block|}
block|}
comment|// If all joins are cross-joins (INNER JOIN ON TRUE), we can use
comment|// we can use comma syntax "FROM a, b, c, d, e".
for|for
control|(
name|Join
name|j2
...
parameter_list|()
lambda|->
literal|"rightResult.neededAlias is null, node is "
operator|+
name|rightResult
operator|.
name|node
...
parameter_list|)
block|{
comment|// If the input is a Sort, wrap SELECT is not required.
specifier|final
name|Result
name|x
decl_stmt|;
...
parameter_list|)
block|{
comment|// groupSet contains at least one column that is not in any groupSet.
comment|// Set of clauses that we expect the builder need to add extra Clause.HAVING
comment|// then can add Having filter condition in buildAggregate.
specifier|final
name|Set
argument_list|<
name|Clause
...
condition|)
block|{
comment|// groupSet contains at least one column that is not in any groupSets.
comment|// To make such columns must appear in the output (their value will
comment|// always be NULL), we generate an extra grouping set, then filter
comment|// it out using a "HAVING GROUPING(groupSets)<> 0".
comment|// We want to generate the
specifier|final
name|SqlNodeList
name|groupingList
init|=
...
name|OTHER
case|:
comment|// Make sure that the group sets contains all bits.
specifier|final
name|List
argument_list|<
name|ImmutableBitSet
...
argument_list|)
decl_stmt|;
comment|// "GROUP BY x, y WITH ROLLUP" implicitly sorts by x, y,
comment|// so skip the ORDER BY.
specifier|final
name|boolean
name|isImplicitlySort
init|=
name|Util
operator|.
name|startsWith
...
return|;
block|}
comment|// MySQL does not allow "WITH ROLLUP" in combination with "ORDER BY",
comment|// so generate the grouped result apply ORDER BY to it.
name|SqlSelect
name|sqlSelect
init|=
name|result
...
parameter_list|()
lambda|->
literal|"modify.getUpdateColumnList() is null for "
operator|+
name|modify
argument_list|)
argument_list|)
...
parameter_list|()
lambda|->
literal|"modify.getSourceExpressionList() is null for "
operator|+
name|modify
argument_list|)
argument_list|)
...
parameter_list|()
lambda|->
literal|"x.neededAlias is null, node is "
operator|+
name|x
operator|.
name|node
