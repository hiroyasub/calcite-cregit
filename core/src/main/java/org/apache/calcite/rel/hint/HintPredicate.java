...
begin_comment
comment|/**  * A {@code HintPredicate} indicates whether a {@link org.apache.calcite.rel.RelNode}  * can apply the specified hint.  *  *<p>Every supported hint should register a {@code HintPredicate}  * into the {@link HintStrategyTable}. For example, {@link HintPredicates#JOIN} implies  * that this hint would be propagated and applied to the {@link org.apache.calcite.rel.core.Join}  * relational expressions.  *  *<p>Usually use {@link NodeTypeHintPredicate} is enough for most of the {@link RelHint}s.  * Some of the hints can only be matched to the relational expression with special  * match conditions(not only the relational expression type).  * i.e. "hash_join(r, st)", this hint can only be applied to JOIN expression that  * has "r" and "st" as the input table names. To implement this, you can make a custom  * {@code HintPredicate} instance.  *  *<p>A {@code HintPredicate} can be used independently or cascaded with other strategies  * with method {@link HintPredicates#and}.  *  *<p>In {@link HintStrategyTable} the predicate is used for  * hints registration.  *  * @see HintStrategyTable  */
end_comment
