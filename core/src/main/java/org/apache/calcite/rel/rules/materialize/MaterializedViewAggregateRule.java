...
begin_comment
comment|/** Materialized view rewriting for aggregate.  *  * @param<C> Configuration type  */
end_comment
...
block|}
block|}
comment|// To simplify things, create an identity topViewProject if not present.
specifier|final
name|Project
name|topViewProject
init|=
...
parameter_list|()
lambda|->
literal|"shuttleReferences produced null for targetNode="
operator|+
name|targetNode
operator|+
literal|", exprsLineage="
operator|+
name|exprsLineage
argument_list|)
decl_stmt|;
...
block|}
block|}
comment|/** Given a relational expression with a single input (such as a Project or    * Aggregate) and the ordinal of an input field, returns the ordinal of the    * output field that references the input field. Or -1 if the field is not    * propagated.    *    *<p>For example, if {@code rel} is {@code Project(c0, c2)} (on input with    * columns (c0, c1, c2)), then {@code find(rel, 2)} returns 1 (c2);    * {@code find(rel, 1)} returns -1 (because c1 is not projected).    *    *<p>If {@code rel} is {@code Aggregate([0, 2], sum(1))}, then    * {@code find(rel, 2)} returns 1, and {@code find(rel, 1)} returns -1.    *    * @param rel Relational expression    * @param ref Ordinal of output field    * @return Ordinal of input field, or -1    */
specifier|private
specifier|static
name|int
name|find
...
continue|continue;
block|}
comment|// We add the CEIL or FLOOR expression to the additional
comment|// expressions, replacing the child expression by the position that
...
argument_list|)
expr_stmt|;
comment|// Then we simplify the expression and we add it to the expressions
comment|// lineage so we can try to find a match.
specifier|final
name|RexNode
name|simplified
init|=
...
return|;
block|}
comment|/** Sets {@link #filterProjectTransposeRule()}. */
name|Config
...
return|;
block|}
comment|/** Sets {@link #filterAggregateTransposeRule()}. */
name|Config
...
return|;
block|}
comment|/** Sets {@link #aggregateProjectPullUpConstantsRule()}. */
name|Config
...
return|;
block|}
comment|/** Sets {@link #projectMergeRule()}. */
name|Config
