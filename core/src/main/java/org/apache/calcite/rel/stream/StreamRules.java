...
block|}
block|}
comment|/**    * Planner rule that converts {@link Delta} over a {@link TableScan} of    * a table other than {@link org.apache.calcite.schema.StreamableTable} to Empty.    */
specifier|public
specifier|static
class|class
name|DeltaTableScanToEmptyRule
...
block|}
block|}
comment|/**    * Planner rule that pushes a {@link Delta} through a {@link Join}.    *    * Product rule [1] is applied to implement the transpose:    * stream(x join y)" = "x join stream(y) union all stream(x) join y    *    * [1] https://en.wikipedia.org/wiki/Product_rule    */
specifier|public
specifier|static
class|class
name|DeltaJoinTransposeRule
