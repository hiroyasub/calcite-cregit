...
expr_stmt|;
block|}
comment|/**    * Infers more equal conditions for the join condition.    *    *<p> For example, in {@code SELECT * FROM T1, T2, T3 WHERE T1.id = T3.id AND T2.id = T3.id},    * we can infer {@code T1.id = T2.id} for the first Join node from second Join node's condition:    * {@code T1.id = T3.id AND T2.id = T3.id}.    *    *<p>For the above SQL, the second Join's condition is {@code T1.id = T3.id AND T2.id = T3.id}.    * After inference, the final condition would be: {@code T1.id = T2.id AND T1.id = T3.id}, the    * {@code T1.id = T2.id} can be further pushed into LHS.    *    * @param rexNodes the Join condition    * @param join the Join node    * @return the newly inferred conditions    */
specifier|protected
name|List
argument_list|<
name|RexNode
...
condition|)
block|{
comment|// Keep the conditions unchanged.
return|return
name|rexNodes
return|;
block|}
...
name|addAll
argument_list|(
name|constructConditionFromEqualSets
argument_list|(
name|join
argument_list|,
name|equalSets
...
condition|)
block|{
comment|// To make the result deterministic.
name|set
operator|=
operator|new
name|LinkedHashSet
...
block|}
block|}
comment|// Add left side conditions.
if|if
condition|(
name|leftSet
operator|.
...
block|}
block|}
comment|// Add right side conditions.
if|if
condition|(
name|rightSet
operator|.
...
block|}
block|}
comment|// Only need one equal condition for each equal set.
if|if
condition|(
name|leftSet
operator|.
