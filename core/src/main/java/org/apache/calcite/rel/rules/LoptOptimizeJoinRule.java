...
begin_comment
comment|/**  * Planner rule that implements the heuristic planner for determining optimal  * join orderings.  *  *<p>It is triggered by the pattern  * {@link org.apache.calcite.rel.logical.LogicalProject}  * ({@link MultiJoin}).  *  * @see CoreRules#MULTI_JOIN_OPTIMIZE  */
end_comment
...
block|{
comment|/** Creates an LoptOptimizeJoinRule. */
specifier|protected
name|LoptOptimizeJoinRule
parameter_list|(
name|Config
...
argument_list|)
argument_list|,
literal|"factorToOffsetMap.get(leftFactor)"
argument_list|)
operator|+
name|leftOffset
expr_stmt|;
...
parameter_list|()
lambda|->
literal|"multiJoin.getOtherSelfJoinFactor("
operator|+
name|factorToAdd
operator|+
literal|") is null"
...
argument_list|)
argument_list|,
literal|"multiJoin.getOuterJoinCond(factorToAdd)"
argument_list|)
expr_stmt|;
block|}
else|else
...
parameter_list|()
lambda|->
literal|"multiJoin.getJoinRemovalFactor(dimIdx) for "
operator|+
name|dimIdx
operator|+
literal|", "
...
block|{
comment|// TODO: is the above if (replacementKeys==null) check placed properly?
name|requireNonNull
argument_list|(
name|replacementKeys
argument_list|,
literal|"replacementKeys"
argument_list|)
expr_stmt|;
name|RelDataTypeField
name|mappedField
...
name|DEFAULT
init|=
name|ImmutableLoptOptimizeJoinRule
operator|.
name|Config
operator|.
name|of
