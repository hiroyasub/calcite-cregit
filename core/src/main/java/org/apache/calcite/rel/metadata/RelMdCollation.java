...
return|;
block|}
comment|// MergeUnion guarantees order, like a sort
return|return
name|copyOf
argument_list|(
name|RelMdCollation
...
block|}
block|}
comment|/** Helper method to determine a {@link Join}'s collation assuming that it    * uses a merge-join algorithm.    *    *<p>If the inputs are sorted on other keys<em>in addition to</em> the join    * key, the result preserves those collations too.    * @deprecated Use {@link #mergeJoin(RelMetadataQuery, RelNode, RelNode, ImmutableIntList, ImmutableIntList, JoinRelType)} */
annotation|@
name|Deprecated
...
parameter_list|)
block|{
comment|// The current implementation always preserve the sort order of the left input
return|return
name|mq
operator|.
name|collations
...
parameter_list|)
block|{
comment|// The current implementation can preserve the sort order of the left input if one of the
comment|// following conditions hold:
comment|// (i) join type is INNER or LEFT;
comment|// (ii) RelCollation always orders nulls last.
specifier|final
name|ImmutableList
argument_list|<
name|RelCollation
