...
argument_list|()
decl_stmt|;
comment|// a. Compute compensation predicate. In this case, we create a NOT
comment|// expression, since the predicate will be used on the query branch
comment|// of the union, while the view branch will produce the rest of the
comment|// results.
name|RexNode
...
comment|// We add a Project on top to ensure the output type of the expression.
name|RelBuilder
name|builder
init|=
name|call
...
decl_stmt|;
name|RexNode
name|rewrittenQueryCompensationPred
init|=
name|rewriteExpression
argument_list|(
name|rexBuilder
...
parameter_list|)
block|{
comment|// TODO: Currently we do not support view partial rewritings for Aggregate operators.
return|return
literal|null
return|;
block|}
...
block|}
comment|//~ Methods ----------------------------------------------------------------
comment|/**    * If the node is an Aggregate, it returns a list of references to the grouping    * columns. Otherwise, it returns a lit of references to all columns in the node.    */
specifier|private
specifier|static
name|List
argument_list|<
...
return|;
block|}
comment|// 2. We check that range intervals for the source are contained in the target.
...
return|;
block|}
comment|// 3. Finally, we check that residual predicates of the source are satisfied
comment|// within the target.
comment|// Compute compensating predicates.
specifier|final
name|RexNode
