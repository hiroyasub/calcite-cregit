...
name|viewNode
decl_stmt|;
comment|// 3.1. Check whether it is a valid view
if|if
condition|(
operator|!
name|isViewMatching
...
expr_stmt|;
block|}
comment|// 3.2. View checks before proceeding
if|if
condition|(
operator|!
name|isValidPlan
...
continue|continue;
block|}
comment|// 3.3. Initialize all query related auxiliary data structures
comment|// that will be used throughout query rewriting process
...
condition|)
block|{
comment|// Currently we only support rewriting with views that use
comment|// the same set of tables than the query, thus we skip it
comment|// TODO: Extend to lift this restriction
continue|continue;
block|}
...
parameter_list|)
function_decl|;
comment|/* This method is responsible for rewriting the query using the given view query.    *    * The input node is a Scan on the view table and possibly a compensation Filter    * on top. If a rewriting can be produced, we return that rewriting. If it cannot    * be produced, we will return null. */
specifier|protected
specifier|abstract
name|RelNode
name|unify
...
condition|)
block|{
comment|// TODO: Rewriting for non-inner joins not supported yet
return|return
literal|false
return|;
block|}
...
return|;
block|}
comment|/* Classifies each of the predicates in the list into one of these three    * categories:    * - column equality predicates, or    * - range predicates, comprising<,<=,>,>=, and = between a reference    * and a constant, or    * - residual predicates, all the rest    *    * For each category, it creates the conjunction of the predicates. The    * result is an array of three RexNode objects corresponding to each    * category. */
specifier|private
specifier|static
name|RexNode
index|[]
...
return|;
block|}
comment|/* Given the equi-column predicates of the query and the view and the    * computed equivalence classes, it extracts possible mappings between    * the equivalence classes.    *    * If there is no mapping, it returns null. If there is a exact match,    * it will return a compensation predicate that evaluates to true.    * Finally, if a compensation predicate needs to be enforced on top of    * the view to make the equivalences classes match, it returns that    * compensation predicate */
specifier|private
specifier|static
name|RexNode
name|generateEquivalenceClasses
...
parameter_list|,
name|RexNode
name|queryEquiColumnsPreds
parameter_list|,
name|EquivalenceClasses
name|qEC
parameter_list|,
name|RexNode
name|viewEquiColumnsPreds
parameter_list|,
name|EquivalenceClasses
name|vEC
parameter_list|)
...
return|;
block|}
comment|/* Given the query and view equivalence classes, it extracts the possible mappings    * from each view equivalence class to each query equivalence class.    *    * If any of the view equivalence classes cannot be mapped to a query equivalence    * class, it returns null. */
specifier|private
specifier|static
name|Mapping
name|extractPossibleMapping
...
return|;
block|}
comment|/* Given the input expression that references source expressions in the query,    * it will rewrite it to refer to the view output.    *    * If any of the subexpressions in the input expression cannot be mapped to    * the query, it will return null.    */
specifier|private
specifier|static
name|RexNode
name|rewriteExpression
...
return|;
block|}
comment|/* Mapping from node expressions to target expressions.    *    * If any of the expressions cannot be mapped, we return null. */
specifier|private
specifier|static
name|Multimap
argument_list|<
