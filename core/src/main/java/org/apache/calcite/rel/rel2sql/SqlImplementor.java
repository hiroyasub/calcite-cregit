...
condition|)
block|{
comment|// If the field is expression and sort by ordinal is set in dialect,
comment|// convert it to ordinal.
return|return
name|SqlLiteral
operator|.
name|createExactNumeric
...
condition|)
block|{
comment|// Cannot merge a Project that contains sort by ordinal under it.
return|return
name|hasSortByOrdinal
argument_list|()
return|;
block|}
if|if
...
return|;
block|}
comment|/**      * Return whether the current {@link SqlNode} in {@link Result} contains sort by column      * in ordinal format.      */
specifier|private
name|boolean
name|hasSortByOrdinal
parameter_list|(
