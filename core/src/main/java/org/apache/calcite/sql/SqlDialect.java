...
name|context
operator|.
name|identifierEscapedQuoteString
argument_list|()
operator|==
literal|null
condition|?
...
condition|)
block|{
comment|// If writer always uses parentheses, it will have started parentheses
comment|// that we now regret. Use a special variant of the operator that does
comment|// not print parentheses, so that we can use the ones already started.
name|operator
operator|=
name|SqlInternalOperators
operator|.
...
block|{
comment|// Use an operator that prints "(a, b, c)" rather than
comment|// "ROW (a, b, c)".
name|operator
operator|=
name|SqlInternalOperators
