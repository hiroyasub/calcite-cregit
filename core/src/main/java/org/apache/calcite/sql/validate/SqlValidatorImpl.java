...
argument_list|()
decl_stmt|;
comment|/**    * Maps a {@link SqlSelect} and a clause to the scope used by that clause.    */
specifier|private
specifier|final
name|Map
argument_list|<
...
argument_list|()
decl_stmt|;
comment|// TypeCoercion instance used for implicit type coercion.
specifier|private
name|TypeCoercion
name|typeCoercion
decl_stmt|;
comment|//~ Constructors -----------------------------------------------------------
comment|/**    * Creates a validator.    *    * @param opTab         Operator table    * @param catalogReader Catalog reader    * @param typeFactory   Type factory    * @param config        Config    */
specifier|protected
name|SqlValidatorImpl
parameter_list|(
name|SqlOperatorTable
...
block|}
block|}
comment|// Only need to try to expand the expr from the left input of join
comment|// since it is always left-deep join.
specifier|final
name|SqlNode
name|node
init|=
...
block|}
block|}
comment|/** Returns the set of field names in the join condition specified by USING    * or implicitly by NATURAL, de-duplicated and in order. */
specifier|public
annotation|@
name|Nullable
name|List
...
condition|)
block|{
name|validateQualifiedCommonColumn
argument_list|(
operator|(
name|SqlJoin
operator|)
...
argument_list|)
expr_stmt|;
comment|// First we should make sure that the first component is the table name.
comment|// Then check whether the qualified identifier contains common column.
for|for
control|(
name|ScopeChild
name|child
...
block|}
block|}
comment|// Only need to try to validate the expr from the left input of join
comment|// since it is always left-deep join.
specifier|final
name|SqlNode
name|node
...
comment|// the list, per standard SQL. Disabled if there are dynamic fields.
if|if
condition|(
operator|!
name|hasDynamicStruct
...
name|EXTEND
case|:
comment|// Until we have a dedicated namespace for EXTEND
name|deriveType
argument_list|(
name|requireNonNull
argument_list|(
...
parameter_list|()
lambda|->
literal|"scope needed to lookup "
operator|+
name|id
argument_list|)
operator|.
...
block|}
block|}
comment|/**    * Namespace for the given node.    * @param node node to compute the namespace for    * @return namespace for the given node, never null    * @see #getNamespace(SqlNode)    */
annotation|@
name|API
argument_list|(
name|since
...
return|;
block|}
comment|/**    * Namespace for the given node.    * @param node node to compute the namespace for    * @param scope namespace scope    * @return namespace for the given node, never null    * @see #getNamespace(SqlNode)    */
annotation|@
name|API
argument_list|(
name|since
...
return|;
block|}
comment|/**    * Namespace for the given node.    * @param id identifier to resolve    * @param scope namespace scope    * @return namespace for the given node, never null    * @see #getNamespace(SqlIdentifier, DelegatingScope)    */
annotation|@
name|API
argument_list|(
name|since
...
block|}
block|}
comment|/**    * Performs expression rewrites which are always used unconditionally. These    * rewrites massage the expression tree into a standard form so that the    * rest of the validation logic can be simpler.    *    *<p>Returns null if and only if the original expression is null.    *    * @param node      expression to be rewritten    * @param underFrom whether node appears directly under a FROM clause    * @return rewritten expression, or null if the original expression is null    */
specifier|protected
annotation|@
name|PolyNull
name|SqlNode
...
condition|)
block|{
comment|// derive type of null literal
name|deriveType
argument_list|(
name|scope
argument_list|,
...
condition|)
block|{
comment|// For AS operator, only infer the operand not the alias
name|inferUnknownTypes
argument_list|(
name|inferredType
argument_list|,
...
block|{
specifier|final
name|UnpivotNamespace
name|namespace
init|=
name|createUnpivotNameSpace
argument_list|(
name|call
argument_list|,
name|enclosingNode
...
name|usingScope
operator|+
literal|", so alias must not be null"
assert|;
name|usingScope
operator|.
name|addChild
...
comment|// column names. We skipped registering it just now.
if|if
condition|(
name|needAliasNamespace
condition|)
...
expr_stmt|;
block|}
comment|// If the operator is SqlWindowTableFunction, restricts the scope as
comment|// its first operand's (the table) scope.
if|if
condition|(
name|operand
operator|instanceof
...
block|}
block|}
comment|// Put the usingScope which can be a JoinScope
comment|// or a SelectScope, in order to see the left items
comment|// of the JOIN tree.
name|scopes
operator|.
name|put
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Register subqueries in the QUALIFY clause
name|registerOperandSubQueries
argument_list|(
name|selectScope
argument_list|,
...
return|;
block|}
comment|/** Returns the parse tree node (GROUP BY, HAVING, or an aggregate function    * call) that causes {@code select} to be an aggregate query, or null if it    * is not an aggregate query.    *    *<p>The node is useful context for error messages,    * but you cannot assume that the node is the only aggregate function. */
specifier|protected
annotation|@
name|Nullable
name|SqlNode
...
return|;
block|}
comment|/** If there is at least one call to an aggregate function, returns the    * first. */
specifier|private
annotation|@
name|Nullable
name|SqlNode
...
condition|)
block|{
comment|// Skip validate if current input table has set semantics
if|if
condition|(
name|tableCharacteristic
operator|.
...
continue|continue;
block|}
comment|// A table function at most has one input table with row semantics
if|if
condition|(
name|visitedRowSemanticsTable
condition|)
...
expr_stmt|;
block|}
comment|// If table function defines the parameter is not table parameter, or is an input table
comment|// parameter with row semantics, then it should not be with PARTITION BY OR ORDER BY.
name|SqlNode
name|currentNode
init|=
name|call
...
condition|)
block|{
comment|// Dig out the underlying operand
name|SqlNode
name|realNode
init|=
operator|(
...
condition|)
block|{
name|throwInvalidRowSemanticsTable
argument_list|(
name|call
argument_list|,
name|idx
...
throw|;
block|}
comment|// Join on fields that occur on each side.
...
block|}
block|}
comment|/** Validates a column in a USING clause, or an inferred join key in a NATURAL join. */
specifier|private
name|void
name|validateCommonJoinColumn
parameter_list|(
...
name|validateCommonInputJoinColumn
argument_list|(
name|id
argument_list|,
name|left
...
parameter_list|()
lambda|->
literal|"unable to find left field "
operator|+
name|name
operator|+
literal|" in "
...
parameter_list|()
lambda|->
literal|"enclosingNode of namespace of "
operator|+
name|child
operator|.
name|name
...
block|}
block|}
comment|/**    * If the {@code node} is a DOT call, returns its first operand. Recurse, if    * the first operand is another DOT call.    *    *<p>In other words, it converts {@code a DOT b DOT c} to {@code a}.    *    * @param node The node to strip DOT    * @return the DOT's first operand    */
specifier|private
specifier|static
name|SqlNode
name|stripDot
...
condition|)
block|{
comment|// we stripped the field access. Recurse to this method, the DOT's operand
comment|// can be another SqlCall, or an SqlIdentifier.
name|checkRollUp
argument_list|(
name|grandParent
argument_list|,
...
parameter_list|()
lambda|->
literal|"window.getDeclName() for "
operator|+
name|window
argument_list|)
decl_stmt|;
...
block|}
name|boolean
name|qualifyContainsWindowFunction
init|=
name|overFinder
operator|.
name|findAgg
...
literal|1
argument_list|,
literal|"sub-query as condition must return only one column"
argument_list|)
expr_stmt|;
name|RelDataTypeField
name|recordField
...
block|{
comment|// Use the field list size to record the field index
comment|// because the select item may be a STAR(*), which could have been expanded.
specifier|final
name|int
name|fieldIdx
init|=
...
argument_list|)
expr_stmt|;
comment|// Refresh the insert row type to keep sync with source.
name|setValidatedNodeType
argument_list|(
name|insert
argument_list|,
...
argument_list|(
block|{
literal|"tableIndexToTargetField"
block|,
literal|"projectMap"
block|}
argument_list|)
...
block|}
block|}
comment|/**    * Check the field count of sql insert source and target node row type.    *    * @param node                    target table sql identifier    * @param table                   target table    * @param strategies              column strategies of target table    * @param targetRowTypeToValidate row type to validate mainly for column strategies    * @param realTargetRowType       target table row type exclusive virtual columns    * @param source                  source node    * @param logicalSourceRowType    source node row type    * @param logicalTargetRowType    logical target row type, contains only target columns if    *                                they are specified or if the sql dialect allows subset insert,    *                                make a subset of fields(start from the left first field) whose    *                                length is equals with the source row type fields number    */
specifier|private
name|void
name|checkFieldCount
parameter_list|(
...
condition|)
block|{
comment|// Allows the source row fields count to be equal with either
comment|// the logical or the real(excludes columns that can not insert into)
comment|// target row fields count.
throw|throw
name|newValidationError
argument_list|(
name|node
...
return|;
block|}
comment|/**    * Checks the type assignment of an INSERT or UPDATE query.    *    *<p>Skip the virtual columns(can not insert into) type assignment    * check if the source fields count equals with    * the real target table fields count, see how #checkFieldCount was used.    *    * @param sourceScope   Scope of query source which is used to infer node type    * @param table         Target table    * @param sourceRowType Source row type    * @param targetRowType Target row type, it should either contain all the virtual columns    *                      (can not insert into) or exclude all the virtual columns    * @param query The query    */
specifier|protected
name|void
name|checkTypeAssignment
parameter_list|(
...
condition|)
block|{
comment|// Returns early if source and target row type equals sans nullability.
return|return;
block|}
if|if
condition|(
...
condition|)
block|{
comment|// Try type coercion first if implicit type coercion is allowed.
name|boolean
name|coerced
init|=
name|typeCoercion
...
block|}
block|}
comment|// Fall back to default behavior: compare the type families.
name|List
argument_list|<
name|RelDataTypeField
argument_list|>
...
block|}
block|}
comment|/**    * Validates snapshot to a table.    *    * @param node  The node to validate    * @param scope Validator scope to derive type    * @param ns    The namespace to lookup table    */
specifier|private
name|void
name|validateSnapshot
parameter_list|(
...
parameter_list|()
lambda|->
literal|"call is null when validating windowOrId "
operator|+
name|windowOrId
argument_list|)
expr_stmt|;
...
parameter_list|()
lambda|->
literal|"order by field is null. All fields: "
operator|+
name|orderBy
argument_list|)
expr_stmt|;
...
assert|;
comment|// Given
comment|//   query PIVOT (agg1 AS a, agg2 AS b, ...
comment|//   FOR (axis1, ..., axisN)
comment|//   IN ((v11, ..., v1N) AS label1,
comment|//       (v21, ..., v2N) AS label2, ...))
comment|// the type is
comment|//   k1, ... kN, a_label1, b_label1, ..., a_label2, b_label2, ...
comment|// where k1, ... kN are columns that are not referenced as an argument to
comment|// an aggregate or as an axis.
comment|// Aggregates, e.g. "PIVOT (sum(x) AS sum_x, count(*) AS c)"
specifier|final
name|List
argument_list|<
name|Pair
...
argument_list|)
expr_stmt|;
comment|// Axes, e.g. "FOR (JOB, DEPTNO)"
specifier|final
name|List
argument_list|<
name|RelDataType
...
expr_stmt|;
block|}
comment|// Columns that have been seen as arguments to aggregates or as axes
comment|// do not appear in the output.
specifier|final
name|Set
argument_list|<
name|String
...
argument_list|)
expr_stmt|;
comment|// Values, e.g. "IN (('CLERK', 10) AS c10, ('MANAGER, 20) AS m20)"
name|pivot
operator|.
...
assert|;
comment|// Given
comment|//   query UNPIVOT ((measure1, ..., measureM)
comment|//   FOR (axis1, ..., axisN)
comment|//   IN ((c11, ..., c1M) AS (value11, ..., value1N),
comment|//       (c21, ..., c2M) AS (value21, ..., value2N), ...)
comment|// the type is
comment|//   k1, ... kN, axis1, ..., axisN, measure1, ..., measureM
comment|// where k1, ... kN are columns that are not referenced as an argument to
comment|// an aggregate or as an axis.
comment|// First, And make sure that each
specifier|final
name|int
name|measureCount
init|=
...
lambda|->
block|{
comment|// Make sure that each (ci1, ... ciM) list has the same arity as
comment|// (measure1, ..., measureM).
if|if
condition|(
name|nodeList
operator|.
...
name|RESOURCE
operator|.
name|unpivotValueArityMismatch
argument_list|(
name|nodeList
operator|.
name|size
...
throw|;
block|}
comment|// Make sure that each (vi1, ... viN) list has the same arity as
comment|// (axis1, ..., axisN).
if|if
condition|(
name|valueList
...
throw|;
block|}
comment|// Make sure that each IN expression is a valid column from the input.
name|nodeList
operator|.
name|forEach
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// What columns from the input are not referenced by a column in the IN
comment|// list?
specifier|final
name|SqlValidatorNamespace
name|inputNs
...
argument_list|)
expr_stmt|;
comment|// What columns will be present in the output row type?
specifier|final
name|Set
argument_list|<
name|String
...
argument_list|)
expr_stmt|;
comment|// Gather the name and type of each measure.
specifier|final
name|List
argument_list|<
name|Pair
...
name|RESOURCE
operator|.
name|unpivotCannotDeriveMeasureType
argument_list|(
name|measureName
argument_list|)
argument_list|)
...
argument_list|)
expr_stmt|;
comment|// Gather the name and type of each axis.
comment|// Consider
comment|//   FOR (job, deptno)
comment|//   IN (a AS ('CLERK', 10),
comment|//       b AS ('ANALYST', 20))
comment|// There are two axes, (job, deptno), and so each value list ('CLERK', 10),
comment|// ('ANALYST', 20) must have arity two.
comment|//
comment|// The type of 'job' is derived as the least restrictive type of the values
comment|// ('CLERK', 'ANALYST'), namely VARCHAR(7). The derived type of 'deptno' is
comment|// the type of values (10, 20), namely INTEGER.
specifier|final
name|List
argument_list|<
name|Pair
...
name|RESOURCE
operator|.
name|unpivotCannotDeriveAxisType
argument_list|(
name|axisName
argument_list|)
argument_list|)
...
name|IGNORED
case|:
comment|// rewrite the order list to empty
if|if
condition|(
name|orderList
operator|!=
...
literal|null
assert|;
comment|// Validate that percentile function have a single ORDER BY expression
if|if
condition|(
name|orderList
operator|.
...
throw|;
block|}
comment|// Validate that the ORDER BY field is of NUMERIC type
name|SqlNode
name|node
init|=
name|orderList
...
return|;
block|}
comment|/** Expands an expression in a GROUP BY, HAVING or QUALIFY clause. */
specifier|private
name|SqlNode
name|extendedExpand
parameter_list|(
...
condition|)
block|{
comment|// If identifier is drawn from a repeated subrecord via unnest, add name of array field
name|UnnestNamespace
name|unnestNamespace
init|=
name|namespace
...
argument_list|()
decl_stmt|;
comment|// NOTE: but parameters on fetch/offset would be counted twice
comment|// as they are counted in the SqlOrderBy call and the inner SqlSelect call
specifier|final
name|Set
argument_list|<
name|SqlNode
...
comment|// for a dynamic star field in dynTable's rowType.
return|return
operator|new
name|SqlBasicCall
argument_list|(
...
parameter_list|()
lambda|->
literal|"OrderExpressionExpander returned null for "
operator|+
name|root
argument_list|)
return|;
...
block|}
block|}
comment|/**    * Converts an expression into canonical form by fully-qualifying any    * identifiers. For common columns in USING, it will be converted to    * COALESCE(A.col, B.col) AS col.    */
specifier|static
class|class
name|SelectExpander
extends|extends
...
return|;
block|}
comment|/**      * Returns whether a given node contains a {@link SqlIdentifier}.      *      * @param sqlNode a SqlNode      * @param target a SqlIdentifier      */
specifier|private
name|boolean
name|containsIdentifier
parameter_list|(
...
parameter_list|()
lambda|->
literal|"NavigationModifier returned for "
operator|+
name|node
argument_list|)
return|;
...
parameter_list|()
lambda|->
literal|"node.accept(PatternValidator) for node "
operator|+
name|node
argument_list|)
argument_list|)
...
block|}
block|}
comment|/** Permutation of fields in NATURAL JOIN or USING. */
specifier|private
class|class
name|Permute
block|{
...
return|;
block|}
comment|/** Moves fields according to the permutation. */
specifier|public
name|void
name|permute
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// output is nullable only if both inputs are
specifier|final
name|boolean
name|nullable
init|=
...
parameter_list|()
lambda|->
literal|"leastRestrictiveForComparison for types "
operator|+
name|currentType
operator|+
literal|" and "
...
name|VALID
block|}
comment|/** Allows {@link #clauseScopes} to have multiple values per SELECT. */
specifier|private
enum|enum
name|Clause
block|{
...
name|QUALIFY
block|;
comment|/**      * Determines if the extender should replace aliases with expanded values.      * For example:      *      *<blockquote><pre>{@code      *  SELECT a + a as twoA      *  GROUP BY twoA      * }</pre></blockquote>      *      *<p>turns into      *      *<blockquote><pre>{@code      *  SELECT a + a as twoA      *  GROUP BY a + a      * }</pre></blockquote>      *      *<p>This is determined both by the clause and the config.      *      * @param config The configuration      * @return Whether we should replace the alias with its expanded value      */
name|boolean
name|shouldReplaceAliases
parameter_list|(
name|Config
