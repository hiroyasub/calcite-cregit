...
argument_list|)
expr_stmt|;
comment|// Skip the virtual columns(can not insert into) type assignment
comment|// check if the source fields num is equals with
comment|// the real target table fields num, see how #checkFieldCount was used.
...
condition|)
block|{
comment|// we allow the source row fields equals with either the target row fields num,
comment|// or the number of real(exclusive columns that can not insert into) target row fields.
throw|throw
name|newValidationError
argument_list|(
name|node
