...
parameter_list|)
function_decl|;
comment|/**    * If an identifier is a legitimate call to a function that has no    * arguments and requires no parentheses (for example "CURRENT_USER"),    * returns a call to that function, otherwise returns null.    */
annotation|@
name|Nullable
name|SqlCall
...
parameter_list|()
function_decl|;
comment|/**    * Saves the type of a {@link SqlNode}, now that it has been validated.    *    *<p>This method is only for internal use. The validator should drive the    * type-derivation process, and store nodes' types when they have been derived.    *    * @param node A SQL parse tree node, never null    * @param type Its type; must not be null    */
annotation|@
name|API
argument_list|(
name|status
...
parameter_list|)
function_decl|;
comment|/** Returns the set of allowed time frames. */
name|TimeFrameSet
name|getTimeFrameSet
parameter_list|()
function_decl|;
comment|/** Validates a time frame.    *    *<p>A time frame is either a built-in time frame based on a time unit such    * as {@link org.apache.calcite.avatica.util.TimeUnitRange#HOUR},    * or is a custom time frame represented by a name in    * {@link SqlIntervalQualifier#timeFrameName}. A custom time frame is    * validated against {@link #getTimeFrameSet()}.    *    *<p>Returns a time frame, or throws.    */
name|TimeFrame
