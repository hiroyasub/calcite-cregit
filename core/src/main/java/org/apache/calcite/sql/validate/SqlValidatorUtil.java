...
argument_list|)
decl_stmt|;
comment|// checkerframework: found   : Set<@KeyFor("indexToField") Integer>
...
return|;
block|}
comment|/**    * Derives an alias for a node, and invents a mangled identifier if it    * cannot.    *    *<p>Examples:    *    *<ul>    *<li>Alias: "1 + 2 as foo" yields "foo"    *<li>Identifier: "foo.bar.baz" yields "baz"    *<li>Anything else yields "expr$<i>ordinal</i>"    *</ul>    *    * @param node   Node    * @param ordinal Ordinal in SELECT clause (must be&ge; 0)    *    * @return An alias, if one can be derived; or a synthetic alias    * "expr$<i>ordinal</i>"; never null    */
specifier|public
specifier|static
name|String
name|alias
...
return|;
block|}
comment|/** Derives an alias for a node, and invents a mangled identifier if it    * cannot.    *    * @deprecated Use {@link #alias(SqlNode)} if {@code ordinal} is negative,    * or {@link #alias(SqlNode, int)} if {@code ordinal} is non-negative. */
annotation|@
name|Deprecated
comment|// to be removed before 2.0
specifier|public
...
return|;
block|}
comment|/** Returns an alias, if one can be derived; or a synthetic alias    * "expr$<i>ordinal</i>" if ordinal&ge; 0; otherwise null. */
specifier|private
specifier|static
annotation|@
name|Nullable
...
begin_comment
comment|/**    * Flattens an aggregate call.    */
end_comment
...
begin_comment
comment|/** Flattens any FILTER, WITHIN DISTINCT, WITHIN GROUP surrounding a call to    * an aggregate function. */
end_comment
