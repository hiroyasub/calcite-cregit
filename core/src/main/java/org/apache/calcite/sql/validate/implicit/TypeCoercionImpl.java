...
parameter_list|)
block|{
comment|// Assert that the operator has NUMERIC family operand type checker.
name|SqlOperator
name|operator
init|=
name|binding
...
argument_list|)
decl_stmt|;
comment|// Special case for datetime +/- interval
if|if
condition|(
name|kind
operator|==
...
block|}
block|}
comment|// Single operand agg function, cast string operand to DECIMAL.
if|if
condition|(
name|binding
operator|.
...
return|;
block|}
comment|// Better to move these functions to a single method.
switch|switch
condition|(
name|kind
condition|)
...
begin_comment
comment|// End TypeCoercionImpl.java
end_comment
