...
block|{
comment|//~ Constants --------------------------------------------------------------
comment|/** Prefix for generated column aliases. Ends with '$' so that human-written    * queries are unlikely to accidentally reference the generated name. */
specifier|public
specifier|static
specifier|final
name|String
...
literal|null
operator|:
literal|"from must not be null for "
operator|+
name|query
assert|;
name|ArrayList
...
return|;
block|}
comment|/** Finds the index of an expression in a list, comparing using    * {@link SqlNode#equalsDeep(SqlNode, Litmus)}. */
specifier|public
specifier|static
name|int
name|indexOfDeep
...
name|CAST
case|:
comment|// "CAST(e AS type)" is literal if "e" is literal
return|return
name|isLiteral
argument_list|(
operator|(
...
literal|true
return|;
comment|// DEFAULT is always NULL
default|default:
return|return
literal|false
return|;
...
return|;
block|}
comment|/**    * Looks up a (possibly overloaded) routine based on name and argument    * types.    *    * @param opTab         operator table to search    * @param typeFactory   Type factory    * @param funcName      name of function being invoked    * @param argTypes      argument types    * @param argNames      argument names, or null if call by position    * @param category      whether a function or a procedure. (If a procedure is    *                      being invoked, the overload rules are simpler.)    * @param nameMatcher   Whether to look up the function case-sensitively    * @param coerce        Whether to allow type coercion when do filter routines    *                      by parameter types    * @return matching routine, or null if none found    *    * @see Glossary#SQL99 SQL:1999 Part 2 Section 10.4    */
specifier|public
specifier|static
annotation|@
name|Nullable
...
comment|// argument types and parameter names if specified
name|routines
operator|=
name|filterRoutinesByParameterTypeAndName
argument_list|(
name|typeFactory
argument_list|,
name|sqlSyntax
...
return|;
block|}
comment|/**    * Permutes argument types to correspond to the order of parameter names.    */
specifier|private
specifier|static
annotation|@
name|Nullable
...
literal|null
operator|:
literal|"fields must not be null in "
operator|+
name|select
assert|;
...
return|;
block|}
comment|/**    * Whether the alias is generated by calcite.    * @param alias not null    * @return true if alias is generated by calcite, otherwise false    */
specifier|public
specifier|static
name|boolean
...
block|}
block|}
comment|/** If a node is "AS", returns the underlying expression; otherwise returns    * the node. Returns null if and only if the node is null. */
specifier|public
specifier|static
annotation|@
name|PolyNull
...
return|;
block|}
comment|/** Modifies a list of nodes, removing AS from each if present.    *    * @see #stripAs */
specifier|public
specifier|static
name|SqlNodeList
name|stripListAs
...
argument_list|()
argument_list|,
literal|"Genealogist result"
argument_list|)
return|;
block|}
block|}
comment|/**    * Returns an immutable list of {@link RelHint} from sql hints, with a given    * inherit path from the root node.    *    *<p>The inherit path would be empty list.    *    * @param hintStrategies The hint strategies to validate the sql hints    * @param sqlHints       The sql hints nodes    * @return the {@code RelHint} list    */
specifier|public
specifier|static
name|List
argument_list|<
...
name|AssertionError
argument_list|(
literal|"Unexpected hint option format"
argument_list|)
throw|;
block|}
specifier|final
...
return|;
block|}
comment|/**    * Attach the {@code hints} to {@code rel} with specified hint strategies.    *    * @param hintStrategies The strategies to filter the hints    * @param hints          The original hints to be attached    * @return A copy of {@code rel} if there are any hints can be attached given    * the hint strategies, or the original node if such hints don't exist    */
specifier|public
specifier|static
name|RelNode
name|attachRelHint
...
return|;
block|}
comment|/** Creates a call to an operator.    *    *<p>Deals with the fact the AND and OR are binary. */
specifier|public
specifier|static
name|SqlNode
name|createCall
...
return|;
block|}
comment|/**    * Creates a balanced binary call from sql node list,    * start inclusive, end exclusive.    */
specifier|private
specifier|static
name|SqlNode
name|createBalancedCall
