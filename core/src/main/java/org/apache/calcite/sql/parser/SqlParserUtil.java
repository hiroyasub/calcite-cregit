...
return|;
block|}
comment|/**    * Converts the contents of a SQL quoted character literal with C-style    * escapes into the corresponding Java string representation.    *    * @throws MalformedUnicodeEscape if input contains invalid unicode escapes    */
specifier|public
specifier|static
name|String
name|parseCString
...
decl_stmt|;
return|return
name|replaceEscapedChars
argument_list|(
name|s2
argument_list|)
return|;
block|}
comment|/**    * Converts the contents of a character literal  with escapes like those used    * in the C programming language to the corresponding Java string    * representation.    *    *<p>If the literal "{@code E'a\tc'}" occurs in the SQL source text, then    * this method will be invoked with the string "{@code a\tc}" (4 characters)    * and will return a Java string with the three characters 'a', TAB, 'b'.    *    *<p>The format is the same as the Postgres; see    *<a href="https://www.postgresql.org/docs/14/sql-syntax-lexical.html#SQL-SYNTAX-CONSTANTS">    * Postgres 4.1.2.2. String Constants With C-Style Escapes</a>.    *    * @param input String that contains C-style escapes    * @return String with escapes converted into Java characters    * @throws MalformedUnicodeEscape if input contains invalid unicode escapes    */
specifier|public
specifier|static
name|String
name|replaceEscapedChars
...
name|MalformedUnicodeEscape
block|{
comment|// The implementation of this method is based on Crate's method
comment|// Literals.replaceEscapedChars.
specifier|final
name|int
name|length
init|=
...
name|endIdx
operator|=
name|calculateMaxCharsInSequence
argument_list|(
name|input
argument_list|,
name|i
...
literal|1
expr_stmt|;
comment|// skip already consumed chars
break|break;
case|case
literal|'x'
case|:
comment|// handle hex byte case - up to 2 chars for hex value
name|endIdx
operator|=
name|calculateMaxCharsInSequence
argument_list|(
...
block|{
comment|// hex sequence unmatched - output original char
name|builder
operator|.
name|append
argument_list|(
...
literal|'3'
case|:
comment|// handle octal case - up to 3 chars
name|endIdx
operator|=
name|calculateMaxCharsInSequence
argument_list|(
...
literal|2
argument_list|,
comment|// first char is already "consumed"
name|SqlParserUtil
operator|::
name|isOctalDigit
argument_list|)
...
break|break;
default|default:
comment|// non-valid escaped char sequence
name|builder
operator|.
name|append
argument_list|(
...
return|;
block|}
comment|/**    * Calculates the maximum number of consecutive characters of the    * {@link CharSequence} argument, starting from {@code beginIndex}, that match    * a given {@link Predicate}. The number of characters to match are either    * capped from the {@code maxCharsToMatch} parameter or the sequence length.    *    *<p>Examples:    *<pre>    * {@code    *    calculateMaxCharsInSequence("12345", 0, 2, Character::isDigit) -> 2    *    calculateMaxCharsInSequence("12345", 3, 2, Character::isDigit) -> 5    *    calculateMaxCharsInSequence("12345", 4, 2, Character::isDigit) -> 5    * }    *</pre>    *    * @return the index of the first non-matching character    */
specifier|private
specifier|static
name|int
name|calculateMaxCharsInSequence
...
return|;
block|}
comment|/**    * Returns true if the specific character is a base-8 digit.    */
specifier|public
specifier|static
name|boolean
name|isOctalDigit
...
return|;
block|}
comment|/**    * Returns true if the specified character is a base-16 digit.    */
specifier|public
specifier|static
name|boolean
name|isHexDigit
...
return|;
block|}
comment|/**    * Parses string to array literal    * using {@link org.apache.calcite.sql.parser.impl.SqlParserImpl} parser.    * String format description can be found at the    *<a href="https://www.postgresql.org/docs/current/arrays.html#ARRAYS-INPUT">link</a>    *    * @param s a string to parse    * @return a array value    *    * @throws SqlParseException if there is a parse error    */
specifier|public
specifier|static
name|SqlNode
...
decl_stmt|;
block|}
comment|/** Thrown by {@link #replaceEscapedChars(String)}. */
specifier|public
specifier|static
class|class
name|MalformedUnicodeEscape
