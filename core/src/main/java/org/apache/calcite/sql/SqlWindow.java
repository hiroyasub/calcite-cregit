...
expr_stmt|;
block|}
comment|/**    * Returns if the window is guaranteed to have rows.    * This is useful to refine data type of window aggregates.    * For instance sum(non-nullable) over (empty window) is NULL.    * @return true when the window is non-empty    * @see org.eigenbase.rel.WindowRelBase.Window#isAlwaysNonEmpty()    * @see SqlOperatorBinding#getGroupCount()    * @see org.eigenbase.sql.validate.SqlValidatorImpl#resolveWindow(SqlNode, org.eigenbase.sql.validate.SqlValidatorScope, boolean)    */
specifier|public
name|boolean
