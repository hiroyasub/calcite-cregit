...
return|;
block|}
comment|/**    * Returns the value of this literal.    *    *<p>Try not to use this method! There are so many different kinds of    * values, it's better to to let SqlLiteral do whatever it is you want to    * do.    *    * @see #booleanValue()    * @see #symbolValue(Class)    */
specifier|public
annotation|@
name|Nullable
name|Object
...
return|;
block|}
comment|/**    * Returns the value of this literal as a given Java type.    *    *<p>Which type you may ask for depends on {@link #typeName}.    * You may always ask for the type where we store the value internally    * (as defined by {@link #valueMatchesType(Object, SqlTypeName)}), but may    * ask for other convenient types.    *    *<p>For example, numeric literals' values are stored internally as    * {@link BigDecimal}, but other numeric types such as {@link Long} and    * {@link Double} are also allowed.    *    *<p>The result is never null. For the NULL literal, returns    * a {@link NullSentinel#INSTANCE}.    *    * @param clazz Desired value type    * @param<T> Value type    * @return Value of the literal in desired type, never null    *    * @throws AssertionError if the value type is not supported    */
specifier|public
parameter_list|<
name|T
extends|extends
...
return|;
block|}
comment|/**    * Extracts the {@link SqlSampleSpec} value from a symbol literal.    *    * @throws ClassCastException if the value is not a symbol literal    * @see #createSymbol(Enum, SqlParserPos)    */
specifier|public
specifier|static
name|SqlSampleSpec
name|sampleValue
...
return|;
block|}
comment|// Literals always have non-null family
switch|switch
condition|(
name|requireNonNull
argument_list|(
...
block|}
block|}
comment|/**    * Extracts the string value from a string literal, a chain of string    * literals, or a CAST of a string literal.    *    * @deprecated Use {@link #value(SqlNode)}    */
annotation|@
name|Deprecated
comment|// to be removed before 2.0
specifier|public
...
return|;
block|}
comment|/**    * Creates a literal which represents a parser symbol, for example the    *<code>TRAILING</code> keyword in the call<code>Trim(TRAILING 'x' FROM    * 'Hello world!')</code>.    *    * @see #symbolValue(Class)    */
specifier|public
specifier|static
name|SqlLiteral
name|createSymbol
...
block|}
block|}
comment|/** Creates a literal whose type is unknown until validation time.    * The literal has a tag that looks like a type name, but the tag cannot be    * resolved until validation time, when we have the mapping from type aliases    * to types.    *    *<p>For example,    *<blockquote>{@code    * TIMESTAMP '1969-07-20 22:56:00'    * }</blockquote>    * calls {@code createUnknown("TIMESTAMP", "1969-07-20 22:56:00")}; at    * validate time, we may discover that "TIMESTAMP" maps to the type    * "TIMESTAMP WITH LOCAL TIME ZONE".    *    * @param tag Type name, e.g. "TIMESTAMP", "TIMESTAMP WITH LOCAL TIME ZONE"    * @param value String encoding of the value    * @param pos Parser position    */
specifier|public
specifier|static
name|SqlLiteral
name|createUnknown
