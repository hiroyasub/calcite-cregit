...
return|;
block|}
comment|/**    * Returns a list of RelNode transformed from all possible combination of    * materialized view uses. Big queries will likely have more than one    * transformed RelNode, e.g., (t1 group by c1) join (t2 group by c2).    * In addition, you can add custom materialized view recognition rules.    * @param rel               the original RelNode    * @param materializations  the materialized view list    * @param materializationRules the materialized view recognition rules    * @return the list of transformed RelNode together with their corresponding    *         materialized views used in the transformation.    */
specifier|public
specifier|static
name|List
argument_list|<
...
name|queryRel
decl_stmt|;
comment|// try to trim unused field in relational expressions.
name|root
operator|=
...
return|;
block|}
comment|/**    * Trim unused fields in relational expressions.    */
specifier|private
specifier|static
name|RelNode
name|trimUnusedfields
