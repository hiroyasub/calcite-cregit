...
begin_comment
comment|/**  * A rule driver that applies rules in a Top-Down manner.  * By ensuring rule applying orders, there could be ways for  * space pruning and rule mutual exclusivity check.  *  *<p>This implementation uses tasks to manage rule matches.  * A Task is a piece of work to be executed, it may apply some rules  * or schedule other tasks.</p>  */
end_comment
...
literal|null
decl_stmt|;
comment|/**    * RelNodes that are generated by {@link org.apache.calcite.rel.PhysicalNode#passThrough}    * or {@link org.apache.calcite.rel.PhysicalNode#derive}. These nodes will not take part    * in another passThrough or derive.    */
specifier|private
specifier|final
name|Set
argument_list|<
...
comment|// Clear the OPTIMIZED state for the RelSubsets and all their ancestors,
...
comment|// either by Register or Reregister. The task driver should schedule tasks
comment|// for the new nodes.
annotation|@
name|Override
specifier|public
name|void
...
comment|// It should be scheduled in the later OptimizeGroup task.
if|if
condition|(
name|applying
operator|==
...
comment|// The upper bound depends on all optimizing RelSubsets that this RelNode belongs to.
comment|// If there are optimizing subsets that come from the same RelSet,
...
block|}
block|}
comment|/**    * Optimizes a RelSubset.    * It schedules optimization tasks for RelNodes in the RelSet.    */
specifier|private
class|class
name|OptimizeGroup
implements|implements
...
comment|// So delay the lower bound check.
...
block|}
block|}
comment|// Always apply O_INPUTS first so as to get a valid upper bound.
for|for
control|(
name|RelNode
name|rel
...
block|}
block|}
comment|/**    * Marks the RelSubset optimized.    * When GroupOptimized returns, the group is either fully    * optimized and has a winner or failed to be optimized.    */
specifier|private
specifier|static
class|class
name|GroupOptimized
...
block|}
block|}
comment|/**    * Optimizes a logical node, including exploring its input and applying rules for it.    */
specifier|private
class|class
name|OptimizeMExpr
implements|implements
...
block|}
block|}
comment|/**    * Ensures that ExploreInputs are working on the correct input group.    * Currently, a RelNode's input may change since Calcite may merge RelSets.    */
specifier|private
class|class
...
control|)
block|{
comment|// Clear the LB cache as exploring state has changed.
name|input
operator|.
name|getCluster
argument_list|()
...
block|}
block|}
comment|/**    * Explores an input for a RelNode.    */
specifier|private
class|class
name|ExploreInput
implements|implements
...
block|}
block|}
comment|/**    * Extracts rule matches from rule queue and adds them to task stack.    */
specifier|private
class|class
name|ApplyRules
implements|implements
...
block|}
block|}
comment|/**    *  Decides how to optimize a physical node.    */
specifier|private
annotation|@
name|Nullable
name|Task
...
return|;
block|}
comment|/**    * Tries to convert the physical node to another trait sets, either by converter rule    * or traits pass through.    */
specifier|private
annotation|@
name|Nullable
name|RelNode
...
return|;
block|}
comment|/**    * Checks whether a node's lower bound is less than a RelSubset's upper bound.    */
specifier|private
name|boolean
name|checkLowerBound
parameter_list|(
...
return|;
block|}
comment|/**    * A task that optimizes input for physical nodes who has only one input.    * This task can be replaced by OptimizeInputs but simplifies lots of logic.    */
specifier|private
class|class
name|OptimizeInput1
implements|implements
...
block|}
block|}
comment|/**    * Optimizes a physical node's inputs.    * This task calculates a proper upper bound for the input and invokes    * the OptimizeGroup task. Group pruning mainly happens here when    * the upper bound for an input is less than the input's lower bound    */
specifier|private
class|class
name|OptimizeInputs
implements|implements
...
block|}
block|}
comment|/**    * Ensures input is optimized correctly and modify context.    */
specifier|private
class|class
name|CheckInput
implements|implements
...
condition|)
block|{
comment|// The input has changed. So reschedule the optimize task.
name|input
operator|=
operator|(
name|RelSubset
...
return|return;
block|}
comment|// Optimizing input completed. Update the context for other inputs.
if|if
condition|(
name|context
operator|==
...
condition|)
block|{
comment|// The input fails to optimize due to group pruning.
...
block|}
block|}
comment|/**    * Derives traits for already optimized physical nodes.    */
specifier|private
class|class
name|DeriveTrait
implements|implements
