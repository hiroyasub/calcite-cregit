...
literal|false
decl_stmt|;
comment|/**    * The upper bound of the last OptimizeGroup call.    */
name|RelOptCost
name|upperBound
decl_stmt|;
comment|/**    * A cache that recognize which RelNode has invoked the passThrough method    * so as to avoid duplicate invocation.    */
annotation|@
name|Nullable
name|Set
argument_list|<
...
block|}
comment|//~ Methods ----------------------------------------------------------------
comment|/**    * Computes the best {@link RelNode} in this subset.    *    *<p>Only necessary when a subset is created in a set that has subsets that    * subsume it. Rationale:</p>    *    *<ol>    *<li>If the are no subsuming subsets, the subset is initially empty.</li>    *<li>After creation, {@code best} and {@code bestCost} are maintained    *    incrementally by {@link VolcanoPlanner#propagateCostImprovements} and    *    {@link RelSet#mergeWith(VolcanoPlanner, RelSet)}.</li>    *</ol>    */
annotation|@
name|EnsuresNonNull
argument_list|(
literal|"bestCost"
...
argument_list|()
argument_list|,
literal|"both best and original nodes are null"
argument_list|)
return|;
block|}
annotation|@
...
return|;
block|}
comment|/**    * Returns the best cost if this subset is fully optimized    * or null if the subset is not fully optimized.    */
annotation|@
name|API
argument_list|(
name|since
...
return|;
block|}
comment|// if bestCost != upperBound, it means optimize failed
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// return memoized result of previous visit if available
return|return
name|prevVisit
return|;
block|}
...
block|}
block_content|)
block|.collect(Collectors.groupingBy(Function.identity(
block_content|)
operator|,
name|Collectors
operator|.
...
begin_comment
comment|// memoize result for pId
end_comment
