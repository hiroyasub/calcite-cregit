...
name|newAggCall
init|=
name|genAggCallWithTargetGrouping
argument_list|(
name|aggregateCall
argument_list|,
name|targetGroupByIndexList
...
block|{
comment|// Here, we create a new `MutableAggregate` to return.
comment|// So, we record this new agg-call.
name|targetGroupGenAggCalls
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// Here, we didn't build target's agg-call by ref of mv's agg-call,
comment|// if some agg-call is generated by target's grouping.
comment|// So, we return null to stop it.
return|return
literal|null
return|;
block|}
...
literal|null
decl_stmt|;
comment|// 1. try to find rollup agg-call.
if|if
condition|(
operator|!
name|aggregateCall
...
condition|)
block|{
comment|// When an SqlAggFunction does not support roll up, it will return null,
comment|// which means that it cannot do secondary aggregation
comment|// and the materialization recognition will fail.
specifier|final
name|SqlAggFunction
name|aggFunction
init|=
...
block|}
block|}
comment|// 2. try to build a new agg-cal by target's grouping.
if|if
condition|(
name|newAggCall
operator|==
...
return|;
block|}
comment|/**    * Generate agg call by mv's grouping.    */
specifier|private
specifier|static
annotation|@
name|Nullable
...
condition|)
block|{
comment|// Size of agg-call's args is empty, we stop to build a new agg-call,
comment|// eg: count(1) or count(*).
name|isAllowBuild
operator|=
literal|false
expr_stmt|;
...
condition|)
block|{
comment|// Args of agg-call is distinct, we can build a new agg-call.
name|isAllowBuild
operator|=
literal|true
expr_stmt|;
...
condition|)
block|{
comment|// If attribute of agg-call's distinct could be ignore, we can build a new agg-call.
name|isAllowBuild
operator|=
literal|true
expr_stmt|;
