...
argument_list|()
decl_stmt|;
comment|/**    * A driver to manage rule and rule matches.    */
name|RuleDriver
name|ruleDriver
decl_stmt|;
...
name|zeroCost
decl_stmt|;
comment|/** Infinite cost, according to {@link #costFactory}. Not necessarily a    * {@link org.apache.calcite.plan.volcano.VolcanoCost}. */
specifier|final
name|RelOptCost
name|infCost
decl_stmt|;
...
argument_list|()
decl_stmt|;
comment|/**    * Extra roots for explorations.    */
name|Set
argument_list|<
name|RelSubset
argument_list|>
...
block|}
comment|//~ Methods ----------------------------------------------------------------
comment|/**    * Enable or disable top-down optimization.    *    *<p>Note: Enabling top-down optimization will automatically enable    * top-down trait propagation.</p>    */
specifier|public
name|void
name|setTopDownOpt
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Returns cost of a relation or infinite cost if the cost is not known.    * @param rel relation t    * @param mq metadata query    * @return cost of the relation or infinite cost if the cost is not known    * @see org.apache.calcite.plan.volcano.RelSubset#bestCost    */
specifier|private
name|RelOptCost
name|getCostOrInfinite
parameter_list|(
...
block|}
block|}
comment|/**    * Returns the subset that a relational expression belongs to.    *    * @param rel Relational expression    * @return Subset it belongs to, or null if it is not registered    * @throws AssertionError in case subset is not found    */
annotation|@
name|API
argument_list|(
name|since
...
parameter_list|()
lambda|->
literal|"Subset is not found for "
operator|+
name|rel
argument_list|)
return|;
...
block|}
block|}
comment|/**    * Checks whether a relexp has made any subset cheaper, and if it so,    * propagate new cost to parent rel nodes.    *    * @param rel       Relational expression whose cost has improved    */
name|void
name|propagateCostImprovements
parameter_list|(
name|RelNode
...
argument_list|)
argument_list|,
literal|"propagateRels.get(relNode)"
argument_list|)
decl_stmt|;
for|for
control|(
...
condition|)
block|{
comment|// Cost reduced, force the heap to adjust its ordering
name|propagateHeap
operator|.
name|remove
argument_list|(
...
name|set2
operator|:
literal|"pre: set1 != set2"
assert|;
comment|// Find the root of each set's equivalence tree.
name|set1
operator|=
name|equivRoot
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// If set1 and set2 are equivalent, there's nothing to do.
if|if
condition|(
name|set2
operator|==
...
condition|)
block|{
comment|// There is a cycle of length 1; each set is the (direct) parent of the
comment|// other. Swap so that we are merging into the larger, older set.
name|swap
operator|=
name|isSmaller
argument_list|(
...
condition|)
block|{
comment|// set2 is a parent of set1. Do not swap. We want to merge set2 into set.
name|swap
operator|=
literal|false
expr_stmt|;
...
condition|)
block|{
comment|// set1 is a parent of set2. Swap, so that we merge set into set2.
name|swap
operator|=
literal|true
expr_stmt|;
...
block|{
comment|// Neither is a parent of the other.
comment|// Swap so that we are merging into the larger, older set.
name|swap
operator|=
name|isSmaller
argument_list|(
...
argument_list|()
return|;
comment|// true if set1 is less popular than set2
block|}
if|if
condition|(
name|set1
...
argument_list|()
return|;
comment|// true if set1 is smaller than set2
block|}
return|return
name|set1
operator|.
...
name|id
return|;
comment|// true if set1 is younger than set2
block|}
specifier|static
name|RelSet
name|equivRoot
...
condition|)
block|{
comment|// The same rel is already registered, so return its subset
return|return
name|getSubsetNonNull
argument_list|(
name|equivExp
...
expr_stmt|;
block|}
comment|/**    * Decide whether a rule is logical or not.    * @param rel The specific rel node    * @return True if the relnode is a logical node    */
annotation|@
name|API
argument_list|(
name|since
...
return|;
block|}
comment|/**    * Gets the upper bound of its inputs.    * Allow users to overwrite this method as some implementations may have    * different cost model on some RelNodes, like Spool.    */
annotation|@
name|API
argument_list|(
name|since
