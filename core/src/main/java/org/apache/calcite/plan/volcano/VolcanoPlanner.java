...
return|;
block|}
comment|// Register all possible combinations of materialization substitution.
comment|// Useful for big queries, e.g.
comment|//   (t1 group by c1) join (t2 group by c2).
specifier|private
name|void
name|useMaterializations
parameter_list|(
name|RelNode
name|root
parameter_list|,
...
block|{
comment|// Based on the assumption that a substitution itself won't trigger another
comment|// substitution, if a materialization is not matched here it won't be useful
comment|// in any subsequent matching for the current level of recursion or this level
comment|// down. So we can safely remove the unmatched materialization from the remnant
comment|// list for the current root.
name|List
argument_list|<
name|RelOptMaterialization
argument_list|>
