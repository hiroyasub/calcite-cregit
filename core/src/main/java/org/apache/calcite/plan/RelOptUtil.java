...
return|;
block|}
comment|/**    * Converts a filter to the new filter that would result if the filter is    * pushed past a LogicalProject that it currently is referencing.    *    * @param filter  the filter to be converted    * @param projRel project rel underneath the filter    * @return converted filter    */
specifier|public
specifier|static
name|RexNode
...
parameter_list|)
block|{
comment|// use RexPrograms to merge the filter and LogicalProject into a
comment|// single program so we can convert the LogicalFilter condition to
comment|// directly reference the LogicalProject's child
name|RexBuilder
name|rexBuilder
init|=
name|projRel
