...
throw|;
block|}
comment|/**    * Copy the {@link org.apache.calcite.rel.hint.RelHint}s from {@code originalRel}    * to {@code newRel} if both of them are {@link Hintable}.    *    *<p>The hints are filtered by the hint strategies when attaching to the {@code newRel}.    *    * @param originalRel Original relational expression    * @param newRel      New relational expression    * @return A copy of {@code newRel} with attached qualified hints from {@code originalRel},    * or {@code newRel} directly if one of them are not {@link Hintable}    */
specifier|public
specifier|static
name|RelNode
name|copyRelHints
