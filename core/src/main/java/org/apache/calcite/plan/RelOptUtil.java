...
return|;
block|}
comment|/**    * Returns a set of variables used by a relational expression or its    * descendants. The set may contain duplicates. The item type is the same as    * {@link org.apache.calcite.rex.RexVariable#getName}    */
specifier|public
specifier|static
name|Set
argument_list|<
...
return|;
block|}
comment|/**    * Creates a plan suitable for use in<code>EXISTS</code> or<code>IN</code>    * statements.    *    * @see org.apache.calcite.sql2rel.SqlToRelConverter#convertExists    *    * @param seekRel    A query rel, for example the resulting rel from 'select *    *                   from emp' or 'values (1,2,3)' or '('Foo', 34)'.    * @param subqueryType Sub-query type    * @param logic  Whether to use 2- or 3-valued boolean logic    * @param needsOuterJoin Whether query needs outer join    *    * @return A pair of a relational expression which outer joins a boolean    * condition column, and a numeric offset. The offset is 2 if column 0 is    * the number of rows and column 1 is the number of rows with not-null keys;    * 0 otherwise.    */
specifier|public
specifier|static
name|Pair
argument_list|<
...
return|;
block|}
comment|/**    * Creates a relational expression which projects a list of (expression, name)    * pairs.    *    * @param child             input relational expression    * @param projectList       list of (expression, name) pairs    * @param optimize          Whether to optimize    * @param projectFactory    Factory to create project operators    */
specifier|public
specifier|static
name|RelNode
name|createProject
...
block|}
block|}
comment|/**    * Pushes down expressions in "equal" join condition.    *    *<p>For example, given    * "emp JOIN dept ON emp.deptno + 1 = dept.deptno", adds a project above    * "emp" that computes the expression    * "emp.deptno + 1". The resulting join condition is a simple combination    * of AND, equals, and input fields, plus the remaining non-equal conditions.    *    * @param originalJoin Join whose condition is to be pushed down    */
specifier|public
specifier|static
name|RelNode
...
return|;
block|}
comment|/**    * Pushes down expressions in "equal" join condition.    *    *<p>For example, given    * "emp JOIN dept ON emp.deptno + 1 = dept.deptno", adds a project above    * "emp" that computes the expression    * "emp.deptno + 1". The resulting join condition is a simple combination    * of AND, equals, and input fields, plus the remaining non-equal conditions.    *    * @param originalJoin Join whose condition is to be pushed down    * @param projectFactory Factory to create project operator    */
specifier|public
specifier|static
name|RelNode
name|pushDownJoinConditions
