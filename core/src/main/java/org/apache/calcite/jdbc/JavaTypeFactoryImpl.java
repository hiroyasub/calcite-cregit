...
condition|)
block|{
comment|// We do not need to change the nullability of the nested fields,
comment|// since it can be overridden by the existing implementation of createTypeWithNullability
comment|// when we treat the nullability of the root struct type.
name|sqlType
operator|=
name|typeFactory
operator|.
