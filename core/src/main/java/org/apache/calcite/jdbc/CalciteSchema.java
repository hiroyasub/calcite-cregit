...
return|;
block|}
comment|/** Creates a snapshot of this CalciteSchema as of the specified time. All    * explicit objects in this CalciteSchema will be copied into the snapshot    * CalciteSchema, while the contents of the snapshot of the underlying schema    * should not change as specified in {@link Schema#snapshot(long)}. Snapshots    * of explicit sub schemas will be created and copied recursively.    *    *<p>Currently, to accommodate the requirement of creating tables on the fly    * for materializations, the snapshot will still use the same table map and    * lattice map as in the original CalciteSchema instead of making copies.</p>    *    * @param now The current time in millis, as returned by    *   {@link System#currentTimeMillis()}    *    * @return the schema snapshot.    */
specifier|public
name|CalciteSchema
name|createSnapshot
parameter_list|(
