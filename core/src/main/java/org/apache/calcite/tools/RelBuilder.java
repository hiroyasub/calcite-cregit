...
return|;
block|}
comment|/**    * Creates a {@link TableSpool} for the most recent relation expression.    *    * @param readType spool's read type (as described in {@link Spool.Type})    * @param writeType spool's write type (as described in {@link Spool.Type})    * @param tableName table name    */
specifier|private
name|RelBuilder
name|tableSpool
parameter_list|(
...
return|;
block|}
comment|/**    * Creates a {@link RepeatUnion} associated to a {@link TransientTable} of the two most recent    * relational expressions on the stack. Warning: if these relational expressions are not    * correctly defined, this operation might lead to an infinite loop.    * The generated {@link RepeatUnion} will:    *   - Evaluate its left term once, propagating the results into the {@link TransientTable}.    *   - Evaluate its right term (which may contain a {@link TableScan} on the    *   {@link TransientTable}) over and over until it produces no more results (or until an    *   optional maximum number of iterations is reached). On each iteration, the results will be    *   propagated into the {@link TransientTable}, overwriting the results from the previous one.    *    * @param tableName name of the {@link TransientTable} associated to the {@link RepeatUnion}    * @param all whether duplicates will be considered or not    * @param maxRep maximum number of iterations, -1 means no limit    */
annotation|@
name|Experimental
specifier|public
name|RelBuilder
...
return|;
block|}
comment|/** Creates a {@link SemiJoin}.    *    * @see #semiJoin(Iterable) */
specifier|public
name|RelBuilder
name|semiJoin
parameter_list|(
...
parameter_list|)
block|{
comment|// There is currently no "AntiJoin" relational expression, so we
comment|// simulate it using a LogicalCorrelate with SemiJoinType.ANTI.
specifier|final
name|RexBuilder
name|rexBuilder
init|=
